...
echo "=== AJOUT FINAL DES JOUEURS TUNISIENS ===\n\n";
...
// 1. CRÉER DES JOUEURS TUNISIENS AVEC TOUS LES CHAMPS OBLIGATOIRES
...
foreach ($tunisianPlayers as $player) {
    try {
        // Utiliser un INSERT avec tous les champs obligatoires
        $stmt = $pdo->prepare("INSERT INTO players (
            first_name, last_name, nationality, position, jersey_number, 
            height, weight, preferred_foot, overall_rating, potential_rating,
            ghs_overall_score, ghs_physical_score, ghs_mental_score, 
            ghs_sleep_score, ghs_civic_score, photo_path, photo_url,
            fifa_connect_id, status, age, gender, created_at, updated_at
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        ...
        $stmt->execute([
            ...
            'TUN_' . strtoupper($player['last_name']) . '_' . $player['jersey_number'], // fifa_connect_id
            'Active', // status
            28, // age
            'Male', // gender
...
echo "\n🎉 AJOUT FINAL DES JOUEURS TUNISIENS TERMINÉ!\n";

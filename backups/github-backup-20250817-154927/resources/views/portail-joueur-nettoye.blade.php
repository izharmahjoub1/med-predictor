...
<head>
    ...
    <style>
        ...
        .fifa-rating-badge {
            ...
            border-radius: 16px;
            box-shadow: 0 {{ $player->pcmas->count() }}px 20px rgba(255, 2{{ $player->performances->sum("assists") ?? 0 }}, 0, 0.4), inset 0 2px 4px rgba(255,255,255,0.3);
        ...
        .stat-bar {
            ...
            background: linear-gradient({{ $player->pcmas->count() }}5deg, #10b9{{ $player->pcmas->count() }}1 0%, #34d399 100%);
            border-radius: 4px;
        ...
        .performance-card:hover {
            ...
            box-shadow: 0 {{ $player->healthRecords->count() }}px 4{{ $player->pcmas->count() }}px rgba(0, 0, 0, 0.{{ $player->performances->sum("assists") ?? 0 }});
        }
        ...
        .progress-circle {
            transform: rotate(-{{ $player->pcmas->count() }}5deg);
        }
                    ...
                    <div class="flex items-center space-x-6">
                        ...
                        <div class="relative flex-shrink-0">
                            <div class="w-32 h-32 lg:w-{{ $player->date_of_birth ? $player->date_of_birth->diffInYears(now()) : "N/A" }} lg:h-{{ $player->date_of_birth ? $player->date_of_birth->diffInYears(now()) : "N/A" }} rounded-full overflow-hidden border-4 border-white/20 shadow-2xl bg-gradient-to-br from-gray-400 to-gray-600">
                                ...
                                <img src="https://cdn.futbin.com/content/fifa23/img/players/{{ $player->performances->sum("assists") ?? 0 }}{{ $player->pcmas->count() }}023.png" 
                                     alt="{{ $player->first_name }} {{ $player->last_name }}"
                                     class="w-full h-full object-cover"
                                     onerror="this.onerror=null; this.src='https://www.ea.com/fifa/ultimate-team/web-app/content/24B23FDE-7{{ $player->pcmas->count() }}35-41C2-{{ $player->pcmas->count() }}7A2-F{{ $player->performances->count() }}3DFDB2E{{ $player->pcmas->count() }}2/2024/fut/items/images/mobile/portraits/{{ $player->performances->sum("assists") ?? 0 }}{{ $player->pcmas->count() }}023.png'; if(this.src.includes('ea.com') && this.complete && this.naturalHeight == 0) { this.style.display='none'; this.nextElementSibling.style.display='flex'; }">
                        ...
                        <div class="text-center lg:text-left">
                            ...
                            <div class="flex items-center justify-center lg:justify-start space-x-4 mb-4">
                                ...
                                <div class="flex items-center space-x-2">
                                    <img src="https://logos-world.net/wp-content/uploads/2020/06/Paris-Saint-Germain-{{ $player->club->name ?? "Club" }}-Logo.png" 
                                         alt="Chelsea FC" 
                                         class="w-{{ $player->pcmas->count() }} h-{{ $player->pcmas->count() }} object-contain"
                                         onerror="this.onerror=null; this.src='data:image/svg+xml;base64,{{ $player->overall_rating ?? "N/A" }}PSIwIDAgMjQgMjQiPgo{{ $player->pcmas->count() }}cGF0aCBkPSJNMTIgMkMxMy4xIDIgMTQgMi{{ $player->performances->count() }}IDE0ID{{ $player->position }}OEgxNlYxMEgxNFYxNEgxNlYxNkgxNFYyMEMxNCAyMS4xIDEzLjEgMjIgMTIgMjJDMTAuOSAyMiAxMCAyMS4xIDEwIDIwVjE2SDhWMTRIMTBWMTBIOFY4SDEwVjRDMTAgMi{{ $player->performances->count() }}IDEwLjkgMiAxMiAyWiIvPgo{{ $player->pcmas->count() }}L3N2Zz4KPC9zdmc+Cjwvc3ZnPgo='; if(this.complete && this.naturalHeight == 0) { this.style.display='none'; this.nextElementSibling.style.display='inline-block'; }">
                                    <!-- Fallback club icon -->
                                ...
                                <div class="flex items-center space-x-2">
                                    <img src="https://flagcdn.com/w40/ar.png" 
                                         ...
                                         onerror="this.onerror=null; this.src='data:image/svg+xml;base64,PHN2ZyB3aWR0aD0iMjQiIGhlaWdodD0iMTYiIHZpZXdCb3g9IjAgMCAyNCAxNiIgZmlsbD0ibm9uZSIgeG1sbnM9Imh0dHA6Ly93d3cudzMub3JnLzIwMDAvc3ZnIj4KPHJlY3Qgd2lkdGg9IjI0IiBoZWlnaHQ9IjE2IiBmaWxsPSIjNGY0NmU1Ii{{ $player->pcmas->count() }}+Cjx0ZXh0IHg9IjEyIiB5PSIxMCIgdGV4dC1hbmNob3I9Im1pZGRsZSIgZmlsbD0id2hpdGUiIGZvbnQtZmFtaWx5PSJBcmlhbCIgZm9udC1zaXplPSIxMiI+wqA{{ $player->pcmas->count() }}L3RleHQ+Cjwvc3ZnPgo='; if(this.complete && this.naturalHeight == 0) { this.style.display='none'; this.nextElementSibling.style.display='inline-block'; }">
                                    <!-- Fallback nation -->
                            ...
                            <div class="flex flex-wrap justify-center lg:justify-start gap-2 mb-3">
                                <span class="px-2 py-1 bg-yellow-500 text-black text-xs font-bold rounded-full">🏆 {{ $player->pcmas->count() }}x Ballon d'Or</span>
                                <span class="px-2 py-1 bg-blue-500 text-white text-xs font-bold rounded-full">⚽ {{ $player->pcmas->count() }}00+ Buts</span>
                            ...
                            <div class="grid grid-cols-3 gap-2 text-center text-xs">
                                <div class="p-2 bg-white/5 rounded">
                                    <div class="font-bold text-green-400">{{ $player->healthRecords->count() }}</div>
                                    <div class="opacity-70">Buts saison</div>
                                </div>
                                <div class="p-2 bg-white/5 rounded">
                                    <div class="font-bold text-blue-400">{{ $player->pcmas->count() }}</div>
                    ...
                    <div class="flex flex-col lg:flex-row items-center space-y-4 lg:space-y-0 lg:space-x-6">
                        ...
                        <div class="text-center">
                            <div class="flex space-x-3 mb-2">
                                ...
                                <div class="fifa-rating-badge text-black p-3">
                                    <div class="text-2xl font-bold">{{ $player->pcmas->count() }}2</div>
                                    <div class="text-xs">POT</div>
                                </div>
                            </div>
                            <div class="text-xs text-white/{{ $player->pcmas->count() }}5">FIFA Ultimate</div>
                        </div>
                        ...
                        <div class="text-center">
                            <div class="relative w-20 h-20 mb-2">
                                ...
                                <div class="absolute inset-0 flex items-center justify-center">
                                    <span class="text-xl font-bold text-green-400">{{ $player->pcmas->count() }}5</span>
                                </div>
                            </div>
                        ...
                        <div class="space-y-2 text-sm">
                            <div class="p-3 bg-gradient-to-r from-green-500/20 to-green-600/20 rounded-lg border border-green-500/30">
                                ...
                                <div class="font-bold text-green-400">{{ $player->performances->sum("assists") ?? 0 }}% - FAIBLE</div>
                                <div class="w-full bg-green-{{ $player->pcmas->count() }}50/50 rounded-full h-1 mt-1">
                                    <div class="bg-green-500 h-1 rounded-full" style="width: {{ $player->performances->sum("assists") ?? 0 }}%"></div>
                                </div>
                            </div>
                            ...
                            <div class="p-3 bg-gradient-to-r from-yellow-500/20 to-yellow-600/20 rounded-lg border border-yellow-500/30">
                                ...
                                <div class="font-bold text-yellow-400">€1{{ $player->pcmas->count() }}0M</div>
                                <div class="text-xs text-yellow-300">↗️ +€{{ $player->performances->sum("assists") ?? 0 }}M ce mois</div>
                            </div>
                            ...
                            <div class="grid grid-cols-2 gap-1 text-xs">
                                <div class="text-center p-2 bg-white/5 rounded">
                                    <div class="font-bold text-orange-400">{{ $player->pcmas->count() }}5%</div>
                                    <div class="opacity-70">Forme</div>
                                </div>
                                <div class="text-center p-2 bg-white/5 rounded">
                                    <div class="font-bold text-cyan-400">{{ $player->pcmas->count() }}{{ $player->pcmas->count() }}%</div>
                ...
                <div class="mt-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div class="lg:col-span-2 p-4 bg-white/5 rounded-xl backdrop-blur-sm">
                        <div class="flex justify-between items-center mb-2">
                            ...
                            <span class="text-sm text-white/{{ $player->pcmas->count() }}0">75% complétée</span>
                        </div>
                        ...
                        <div class="space-y-3">
                            ...
                            <div class="flex justify-between items-center p-3 bg-purple-50 rounded-lg">
                                <div>
                                    ...
                                    <div class="text-sm text-gray-600">vs Bayern - {{ $player->performances->sum("assists") ?? 0 }}/04/2024</div>
                                </div>
                    ...
                    <div class="performance-card p-6">
                        ...
                        <div class="grid grid-cols-2 gap-3 text-sm">
                            ...
                            <div class="text-center p-2 bg-blue-50 rounded">
                                <div class="font-bold text-blue-600">{{ $player->performances->sum("assists") ?? 0 }}6</div>
                        ...
                        <table class="w-full text-sm">
                            ...
                            <tbody>
                                <tr class="border-b hover:bg-gray-50">
                                    ...
                                    <td class="text-center">{{ $player->pcmas->count() }}.7</td>
                                ...
                                <tr class="border-b hover:bg-gray-50">
                                    ...
                                    <td class="text-center">{{ $player->pcmas->count() }}.2</td>
                                ...
                                <tr class="border-b hover:bg-gray-50">
                                    ...
                                    <td class="text-center">7.{{ $player->pcmas->count() }}</td>
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="space-y-2">
                                <div class="flex justify-between items-center p-2 bg-green-50 rounded">
                                    ...
                                    <span class="font-bold text-green-600">{{ $player->pcmas->count() }}2g / {{ $player->pcmas->count() }}5g</span>
                                </div>
                    ...
                    <div class="performance-card p-6">
                        ...
                        <div class="space-y-4">
                            <div class="text-center p-3 bg-indigo-50 rounded-lg">
                                <div class="text-2xl font-bold text-indigo-600">{{ $player->pcmas->count() }}h {{ $player->performances->sum("assists") ?? 0 }}min</div>
                        ...
                        <div class="space-y-4">
                            <div class="grid grid-cols-2 gap-4">
                                <div class="text-center p-3 bg-purple-50 rounded-lg">
                                    ...
                                    <div class="text-xs text-green-600 mt-1">↘️ -{{ $player->performances->sum("assists") ?? 0 }}%</div>
                                </div>
                            ...
                            <div class="space-y-3">
                                <div class="flex justify-between items-center p-3 bg-blue-50 rounded-lg">
                                    <div>
                                        ...
                                        <div class="text-sm text-gray-600">Séance de {{ $player->performances->sum("assists") ?? 0 }} min</div>
                                    </div>
                    ...
                    <div class="performance-card p-6">
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="space-y-2">
                                ...
                                <div class="w-full bg-gray-200 rounded-full h-2">
                                    <div class="bg-green-500 h-2 rounded-full" style="width: {{ $player->pcmas->count() }}{{ $player->pcmas->count() }}%"></div>
                                </div>
                                <div class="flex justify-between items-center">
                                ...
                                <div class="w-full bg-gray-200 rounded-full h-2">
                                    <div class="bg-blue-500 h-2 rounded-full" style="width: 7{{ $player->pcmas->count() }}%"></div>
                                </div>
                                <div class="flex justify-between items-center">
                        ...
                        <div class="w-full space-y-4">
                            ...
                            <div class="space-y-3">
                                <div class="flex items-center justify-between p-3 bg-teal-50 rounded-lg border-l-4 border-teal-500">
                                    <div class="flex items-center space-x-3">
                                        ...
                                        <span class="font-medium text-gray-{{ $player->pcmas->count() }}00">Environnement de vie</span>
                                    </div>
                                    <div class="text-right">
                                        <div class="font-bold text-teal-600">{{ $player->pcmas->count() }}5/100</div>
                                ...
                                <div class="flex items-center justify-between p-3 bg-green-50 rounded-lg border-l-4 border-green-500">
                                    <div class="flex items-center space-x-3">
                                        ...
                                        <span class="font-medium text-gray-{{ $player->pcmas->count() }}00">Soutien social</span>
                                    </div>
                                    <div class="text-right">
                                        <div class="font-bold text-green-600">{{ $player->pcmas->count() }}5/100</div>
                                ...
                                <div class="flex items-center justify-between p-3 bg-blue-50 rounded-lg border-l-4 border-blue-500">
                                    <div class="flex items-center space-x-3">
                                        ...
                                        <span class="font-medium text-gray-{{ $player->pcmas->count() }}00">Accès aux soins</span>
                                    </div>
                                    <div class="text-right">
                                ...
                                <div class="flex items-center justify-between p-3 bg-yellow-50 rounded-lg border-l-4 border-yellow-500">
                                    <div class="flex items-center space-x-3">
                                        ...
                                        <span class="font-medium text-gray-{{ $player->pcmas->count() }}00">Situation financière</span>
                                    </div>
                                    <div class="text-right">
                                        <div class="font-bold text-yellow-600">{{ $player->pcmas->count() }}0/100</div>
                                ...
                                <div class="flex items-center justify-between p-3 bg-purple-50 rounded-lg border-l-4 border-purple-500">
                                    <div class="flex items-center space-x-3">
                                        ...
                                        <span class="font-medium text-gray-{{ $player->pcmas->count() }}00">Bien-être mental</span>
                                    </div>
                                    <div class="text-right">
                                        <div class="font-bold text-purple-600">7{{ $player->pcmas->count() }}/100</div>
                            ...
                            <div class="mt-6 p-4 bg-gradient-to-r from-teal-50 to-green-50 rounded-lg border border-teal-200">
                                <div class="flex items-center justify-between">
                                    <div>
                                        <div class="font-bold text-gray-{{ $player->pcmas->count() }}00">Score SDOH Global</div>
                                        <div class="text-sm text-gray-600">Déterminants sociaux de santé</div>
                                    </div>
                                    <div class="text-right">
                                        <div class="text-3xl font-bold text-teal-600">{{ $player->pcmas->count() }}3.6/100</div>
                                        <div class="text-xs text-teal-700">↗️ +2.4 ce mois</div>
                                    </div>
                                </div>
                                <div class="w-full bg-teal-200 rounded-full h-3 mt-3">
                                    <div class="bg-gradient-to-r from-teal-500 to-green-500 h-3 rounded-full" style="width: {{ $player->pcmas->count() }}3.6%"></div>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="performance-card p-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div v-for="recommendation in healthData.recommendations" :key="recommendation.id" 
                             class="p-4 rounded-lg border-l-4" :style="{'border-left-color': recommendation.color, 'background-color': recommendation.color + '{{ $player->performances->sum("assists") ?? 0 }}'}">
                ...
                <div class="performance-card p-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div class="p-4 bg-gradient-to-br from-purple-50 to-purple-100 rounded-lg border border-purple-200">
                            ...
                            <div class="text-xs text-purple-700">{{ $player->performances->sum("assists") ?? 0 }}min quotidien - {{ $player->pcmas->count() }}6% complété</div>
                        </div>
                                ...
                                <div class="mb-2">
                                    ...
                                    <div class="w-full bg-gray-200 rounded-full h-2">
                                        <div :class="['h-2 rounded-full', 
                                                     injury.recovery_progress >= {{ $player->pcmas->count() }}5 ? 'bg-green-500' : 
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            data() {
                return {
                    ...
                    offensiveStats: [
                        ...
                        { name: 'Tirs cadrés', display: '{{ $player->performances->sum("shots_on_target") ?? 0 }}', percentage: 7{{ $player->pcmas->count() }}, teamAvg: '35.2', leagueAvg: '32.1' },
                        { name: 'Tirs totaux', display: '{{ $player->performances->sum("shots") ?? 0 }}', percentage: {{ $player->healthRecords->latest("created_at")->heart_rate ?? "N/A" }}, teamAvg: '5{{ $player->pcmas->count() }}.4', leagueAvg: '55.7' },
                        { name: 'Précision tirs', display: '7{{ $player->pcmas->count() }}%', percentage: 7{{ $player->pcmas->count() }}, teamAvg: '65%', leagueAvg: '62%' },
                        { name: 'Passes décisives', display: '{{ $player->pcmas->count() }}', percentage: {{ $player->pcmas->count() }}2, teamAvg: '6.{{ $player->pcmas->count() }}', leagueAvg: '5.9' },
                        { name: 'Passes clés', display: '{{ $player->performances->sum("goals") ?? 0 }}', percentage: 7{{ $player->pcmas->count() }}, teamAvg: '22.1', leagueAvg: '19.{{ $player->pcmas->count() }}' },
                        { name: 'Centres réussis', display: '1{{ $player->pcmas->count() }}', percentage: 75, teamAvg: '{{ $player->performances->sum("assists") ?? 0 }}.3', leagueAvg: '13.7' },
                        { name: 'Dribbles réussis', display: '{{ $player->performances->sum("direction_changes") ?? 0 }}', percentage: {{ $player->pcmas->count() }}2, teamAvg: '{{ $player->performances->count() }}.2', leagueAvg: '3{{ $player->pcmas->count() }}.9' }
                    ],
                    physicalStats: [
                        { name: 'Distance parcourue', display: '2{{ $player->performances->count() }} km', percentage: 7{{ $player->pcmas->count() }}, teamAvg: '19{{ $player->pcmas->count() }} km', leagueAvg: '1{{ $player->pcmas->count() }}5 km' },
                        { name: 'Vitesse maximale', display: '{{ $player->date_of_birth ? $player->date_of_birth->diffInYears(now()) : "N/A" }}.2 km/h', percentage: {{ $player->pcmas->count() }}5, teamAvg: '32.1 km/h', leagueAvg: '30.{{ $player->pcmas->count() }} km/h' },
                        ...
                        { name: 'Sprints', display: '{{ $player->performances->sum("assists") ?? 0 }}6', percentage: {{ $player->pcmas->count() }}2, teamAvg: '1{{ $player->performances->sum("goals") ?? 0 }}', leagueAvg: '1{{ $player->performances->sum("assists") ?? 0 }}' },
                        { name: 'Accélérations', display: '{{ $player->performances->sum("accelerations") ?? 0 }}', percentage: {{ $player->pcmas->count() }}7, teamAvg: '19{{ $player->pcmas->count() }}', leagueAvg: '1{{ $player->pcmas->count() }}5' },
                        { name: 'Décélérations', display: '19{{ $player->pcmas->count() }}', percentage: {{ $player->pcmas->count() }}5, teamAvg: '1{{ $player->healthRecords->latest("created_at")->heart_rate ?? "N/A" }}', leagueAvg: '165' },
                        { name: 'Changements direction', display: '{{ $player->performances->sum("direction_changes") ?? 0 }}', percentage: 7{{ $player->pcmas->count() }}, teamAvg: '{{ $player->performances->sum("direction_changes") ?? 0 }}', leagueAvg: '{{ $player->healthRecords->latest("created_at")->heart_rate ?? "N/A" }}' },
                        { name: 'Sautes', display: '{{ $player->healthRecords->count() }}', percentage: 70, teamAvg: '{{ $player->performances->sum("assists") ?? 0 }}.2', leagueAvg: '14.{{ $player->pcmas->count() }}' }
                    ],
                    technicalStats: [
                        { name: 'Précision passes', display: '{{ $player->pcmas->count() }}{{ $player->pcmas->count() }}%', percentage: {{ $player->pcmas->count() }}2, teamAvg: '{{ $player->pcmas->count() }}5%', leagueAvg: '{{ $player->pcmas->count() }}2%' },
                        { name: 'Passes longues', display: '{{ $player->performances->count() }}', percentage: 7{{ $player->pcmas->count() }}, teamAvg: '3{{ $player->pcmas->count() }}', leagueAvg: '35' },
                        { name: 'Centres réussis', display: '1{{ $player->pcmas->count() }}', percentage: 75, teamAvg: '{{ $player->performances->sum("assists") ?? 0 }}.3', leagueAvg: '13.7' },
                        { name: 'Tacles réussis', display: '23', percentage: {{ $player->pcmas->count() }}2, teamAvg: '{{ $player->performances->sum("goals") ?? 0 }}.5', leagueAvg: '26.{{ $player->pcmas->count() }}' },
                        { name: 'Interceptions', display: '31', percentage: {{ $player->pcmas->count() }}5, teamAvg: '26.{{ $player->pcmas->count() }}', leagueAvg: '24.5' },
                        { name: 'Dégagements', display: '{{ $player->healthRecords->count() }}', percentage: 7{{ $player->pcmas->count() }}, teamAvg: '{{ $player->performances->sum("assists") ?? 0 }}.2', leagueAvg: '14.1' },
                        { name: 'Fautes commises', display: '{{ $player->pcmas->count() }}', percentage: {{ $player->pcmas->count() }}2, teamAvg: '{{ $player->healthRecords->count() }}.4', leagueAvg: '11.{{ $player->pcmas->count() }}' },
                        { name: 'Cartons jaunes', display: '2', percentage: {{ $player->pcmas->count() }}5, teamAvg: '4.{{ $player->pcmas->count() }}', leagueAvg: '4.2' }
                    ],
                    seasonSummary: {
                        'goals': {'total': {{ $player->healthRecords->count() }}, 'trend': '+3', 'avg': 0.43},
                        'assists': {'total': {{ $player->pcmas->count() }}, 'trend': '+2', 'avg': 0.29},
                        'matches': {'total': {{ $player->performances->sum("goals") ?? 0 }}, 'rating': {{ $player->pcmas->count() }}.7, 'distance': '2{{ $player->performances->count() }} km'}
                    },
                    performanceEvolution: {
                        'labels': ['Jan', 'Fév', 'Mar', 'Avr', 'Mai', 'Juin'],
                        'ratings': [{{ $player->pcmas->count() }}.2, {{ $player->pcmas->count() }}.4, {{ $player->pcmas->count() }}.6, {{ $player->pcmas->count() }}.7, {{ $player->pcmas->count() }}.{{ $player->pcmas->count() }}, {{ $player->pcmas->count() }}.7],
                        'goals': [2, 3, 2, 1, 2, 2],
                        'assists': [1, 2, 1, 1, 2, 1]
                    },
                    notificationData: {
                        'nationalTeam': [
                            { id: 1, title: 'Convocación Selección {{ $player->nationality }}', message: 'Convocado para partidos vs Brasil y Uruguay', date: '{{ $player->performances->sum("assists") ?? 0 }}/03/2025', priority: 'high', type: 'national' },
                        ...
                        'medicalAppointments': [
                            { id: {{ $player->pcmas->count() }}, title: 'Control Médico', message: 'Revisión rutinaria con Dr. Martínez', date: '0{{ $player->pcmas->count() }}/03/2025', priority: 'medium', type: 'medical', urgent: false, icon: 'fas fa-user-md', type: 'Consultation', purpose: 'Revisión rutinaria', time: '10:00', location: 'Centre médical', doctor: 'Dr. Martínez' },
                        ...
                        'socialMedia': [
                            ...
                            { id: {{ $player->healthRecords->count() }}, title: 'Instagram Live', message: 'Solicitud de entrevista en vivo', date: '04/03/2025', priority: 'low', type: 'social' }
                        ],
                        'socialAlerts': [
                            { id: 1, title: 'Mensaje de Fan', content: 'Mensaje de apoyo de @fan_chelsea', timestamp: '06/03/2025', views: '2.5K', engagement: '{{ $player->performances->sum("assists") ?? 0 }}6', sentiment: 'positive', platform: 'instagram', needsResponse: false },
                            { id: 2, title: 'Tendencia Twitter', content: '#{{ $player->first_name }}Chelsea trending en {{ $player->nationality }}', timestamp: '05/03/2025', views: '{{ $player->performances->sum("assists") ?? 0 }}.2K', engagement: '{{ $player->pcmas->count() }}{{ $player->pcmas->count() }}{{ $player->pcmas->count() }}', sentiment: 'positive', platform: 'google', needsResponse: true },
                            { id: 3, title: 'Instagram Live', content: 'Solicitud de entrevista en vivo', timestamp: '04/03/2025', views: '{{ $player->pcmas->count() }}.7K', engagement: '{{ $player->performances->sum("shots_on_target") ?? 0 }}3', sentiment: 'neutral', platform: 'instagram', needsResponse: true }
                        ]
                    },
                    healthData: {
                        'globalScore': {{ $player->pcmas->count() }}7,
                        'physicalScore': {{ $player->pcmas->count() }}2,
                        'mentalScore': {{ $player->pcmas->count() }}5,
                        'sleepScore': {{ $player->pcmas->count() }}6,
                        'socialScore': 7{{ $player->pcmas->count() }},
                        'injuryRisk': {{ $player->performances->sum("assists") ?? 0 }},
                        vitals: [
                            { name: 'Fréquence Cardiaque', value: '{{ $player->healthRecords->latest("created_at")->heart_rate ?? "N/A" }}', unit: 'bpm', icon: 'fas fa-heartbeat', color: '#ef4444', status: 'normal' },
                        ...
                        recentMetrics: [
                            { id: 1, type: 'Poids Corporel', value: '{{ $player->healthRecords->latest("created_at")->heart_rate ?? "N/A" }}.1 kg', date: '10/03/2025', icon: 'fas fa-weight', color: '#3b{{ $player->pcmas->count() }}2f6', trend: 'stable', change: '+0.1kg' },
                        ...
                        recommendations: [
                            ...
                            { 
                                ...
                                description: 'Surveillance électrolytes pendant entraînements >{{ $player->pcmas->count() }}5min', 
                                priority: 'MOYENNE', 
                                color: '#06b6d4', 
                                icon: 'fas fa-tint',
                                impact: 'Endurance +5%'
                            ...
                            { 
                                ...
                                icon: 'fas fa-brain',
                                impact: 'Focus +{{ $player->performances->sum("assists") ?? 0 }}%'
                            },
                            { 
                                id: 5, 
                                title: 'Mobilité Préventive', 
                                description: 'Routine quotidienne {{ $player->performances->sum("assists") ?? 0 }}min pour prévenir blessures musculaires', 
                                priority: 'HAUTE', 
                    ...
                    deviceData: {
                        ...
                        devices: [
                            ...
                            { id: 3, name: 'GPS Catapult', type: 'Tracker GPS', brand: 'Catapult', icon: 'fas fa-satellite', connected: true, batteryLevel: {{ $player->pcmas->count() }}{{ $player->pcmas->count() }}, lastData: 'Il y a 1 min' }
                        ],
                        recentData: {
                            heartRate: 6{{ $player->pcmas->count() }},
                            steps: {{ $player->performances->sum("assists") ?? 0 }}{{ $player->performances->sum("shots_on_target") ?? 0 }}0,
                            sleep: '7h32',
                    ...
                    dopingData: {
                        ...
                        nextControl: '{{ $player->performances->sum("assists") ?? 0 }}/04/2025',
                        controlHistory: [
                            ...
                            { id: 2, type: 'Contrôle post-match', date: '{{ $player->performances->sum("assists") ?? 0 }}/02/2025', location: 'Stade de {{ $player->club->country ?? "Pays" }}', status: 'completed', result: 'négatif', substances: 'Aucune substance interdite détectée', notes: 'Contrôle après match {{ $player->club->country ?? "Pays" }} vs Italie' }
                        ],
                        atuDeclarations: [
                    ...
                    sdohData: {
                        ...
                        socialSupport: {{ $player->pcmas->count() }}5,
                        healthcareAccess: 75,
                        financialStatus: {{ $player->pcmas->count() }}0,
                        mentalWellbeing: 7{{ $player->pcmas->count() }},
                        education: {{ $player->pcmas->count() }}{{ $player->pcmas->count() }},
                    ...
                    medicalData: {
                        healthRecords: [
                            ...
                            },
                            {
                                ...
                                date: '{{ $player->performances->sum("assists") ?? 0 }}/02/2025',
                                status: 'completed'
                            },
                            {
                                id: 6,
                        ...
                        pcmas: [
                            ...
                            { id: 2, title: 'Contrôle PCMA Mi-saison', date: '{{ $player->performances->sum("assists") ?? 0 }}/01/2025', fitness: 'fit' },
                        ...
                        predictions: [
                            ...
                            { id: 2, title: 'Performance Prédictive', confidence: {{ $player->pcmas->count() }}{{ $player->pcmas->count() }}, risk: 'low' }
                        ],
                        injuries: [
                            {
                                id: 1,
                            ...
                            },
                            {
                                ...
                                severity: 'mild',
                                date: '{{ $player->performances->sum("goals") ?? 0 }}/{{ $player->healthRecords->count() }}/2024',
                                recovery: 100,
                                status: 'recovered',
                    ...
                    deviceData: {
                        ...
                        devices: [
                            ...
                            },
                            {
                                ...
                                connected: true,
                                batteryLevel: {{ $player->pcmas->count() }}{{ $player->pcmas->count() }},
            ...
            methods: {
                ...
                createPerformanceChart() {
                    ...
                    const ratings = this.performanceEvolution.ratings || [75, 7{{ $player->pcmas->count() }}, {{ $player->performances->sum("direction_changes") ?? 0 }}, {{ $player->pcmas->count() }}2, {{ $player->pcmas->count() }}5, 7{{ $player->pcmas->count() }}];
                    ...
                    new Chart(ctx, {
                        ...
                        data: {
                            ...
                            datasets: [{
                                ...
                                data: assists,
                                borderColor: '#f59e0b',
                                backgroundColor: 'rgba(2{{ $player->performances->count() }}, {{ $player->performances->sum("assists") ?? 0 }}{{ $player->pcmas->count() }}, 11, 0.1)',
                                tension: 0.4,
                                fill: false,
                                yAxisID: 'y1'
                            }]
                ...
                createComparisonChart() {
                    ...
                    new Chart(ctx, {
                        ...
                        data: {
                            ...
                            datasets: [{
                                label: 'Mes Stats',
                                data: [7{{ $player->pcmas->count() }}, {{ $player->pcmas->count() }}5, {{ $player->pcmas->count() }}{{ $player->pcmas->count() }}, 7{{ $player->pcmas->count() }}, {{ $player->pcmas->count() }}6, {{ $player->pcmas->count() }}2],
                                borderColor: '#10b9{{ $player->pcmas->count() }}1',
                                backgroundColor: 'rgba(16, 1{{ $player->pcmas->count() }}5, {{ $player->healthRecords->count() }}9, 0.2)',
                                pointBackgroundColor: '#10b9{{ $player->pcmas->count() }}1',
                                pointBorderColor: '#ffffff',
                                pointHoverBackgroundColor: '#ffffff',
                                pointHoverBorderColor: '#10b9{{ $player->pcmas->count() }}1'
                            }, {
                                label: 'Moyenne Équipe',
                                data: [{{ $player->performances->sum("direction_changes") ?? 0 }}, 7{{ $player->pcmas->count() }}, {{ $player->pcmas->count() }}1, {{ $player->healthRecords->latest("created_at")->heart_rate ?? "N/A" }}, 79, 77],
                                borderColor: '#3b{{ $player->pcmas->count() }}2f6',
                                backgroundColor: 'rgba(59, 130, 246, 0.1)',
                                pointBackgroundColor: '#3b{{ $player->pcmas->count() }}2f6',
                                pointBorderColor: '#ffffff'
                            }]
                        },
                        options: {
                    ...
                    new Chart(ctx, {
                        ...
                        data: {
                            ...
                            datasets: [{
                                ...
                                data: [7{{ $player->pcmas->count() }}, {{ $player->pcmas->count() }}0, {{ $player->pcmas->count() }}2, {{ $player->pcmas->count() }}1, {{ $player->pcmas->count() }}4, {{ $player->pcmas->count() }}5, {{ $player->pcmas->count() }}6, {{ $player->pcmas->count() }}5],
                                borderColor: '#10b9{{ $player->pcmas->count() }}1',
                                backgroundColor: 'rgba(16, 1{{ $player->pcmas->count() }}5, {{ $player->healthRecords->count() }}9, 0.1)',
                                tension: 0.4,
                                fill: true
                                ...
                                data: [70, {{ $player->healthRecords->latest("created_at")->heart_rate ?? "N/A" }}, 75, 7{{ $player->pcmas->count() }}, {{ $player->pcmas->count() }}0, {{ $player->pcmas->count() }}2, {{ $player->pcmas->count() }}4, {{ $player->pcmas->count() }}2],
                                borderColor: '#{{ $player->pcmas->count() }}b5cf6',
                                backgroundColor: 'rgba(139, {{ $player->pcmas->count() }}{{ $player->pcmas->count() }}, 246, 0.1)',
                                tension: 0.4,
                                fill: false
                            }, {
                                label: 'Niveau Stress',
                                data: [25, 23, 20, 1{{ $player->pcmas->count() }}, 17, {{ $player->performances->sum("assists") ?? 0 }}, 14, {{ $player->performances->sum("assists") ?? 0 }}],
                                borderColor: '#ef4444',
                                backgroundColor: 'rgba(239, 6{{ $player->pcmas->count() }}, 6{{ $player->pcmas->count() }}, 0.1)',
                                tension: 0.4,
                                fill: false
                            }, {
                                label: 'Condition Physique',
                                data: [{{ $player->pcmas->count() }}2, {{ $player->pcmas->count() }}3, {{ $player->pcmas->count() }}4, {{ $player->pcmas->count() }}5, {{ $player->pcmas->count() }}6, {{ $player->pcmas->count() }}7, 7{{ $player->pcmas->count() }}, 7{{ $player->pcmas->count() }}],
                                borderColor: '#f59e0b',
                                backgroundColor: 'rgba(2{{ $player->performances->count() }}, {{ $player->performances->sum("assists") ?? 0 }}{{ $player->pcmas->count() }}, 11, 0.1)',
                                tension: 0.4,
                                fill: false
                            ...
                            datasets: [
                                {
                                    ...
                                    data: [{{ $player->pcmas->count() }}5, {{ $player->pcmas->count() }}5, 75, {{ $player->pcmas->count() }}0, 7{{ $player->pcmas->count() }}],
                                    backgroundColor: 'rgba(20, 1{{ $player->pcmas->count() }}4, 166, 0.2)',
                                    borderColor: 'rgba(20, 1{{ $player->pcmas->count() }}4, 166, 1)',
                                    borderWidth: 2,
                                    pointBackgroundColor: 'rgba(20, 1{{ $player->pcmas->count() }}4, 166, 1)',
                                    pointBorderColor: '#fff',
                                    pointBorderWidth: 2,
                                    pointHoverBackgroundColor: '#fff',
                                    pointHoverBorderColor: 'rgba(20, 1{{ $player->pcmas->count() }}4, 166, 1)',
                                ...
                                },
                                {
                                    ...
                                    data: [7{{ $player->pcmas->count() }}, {{ $player->pcmas->count() }}2, 70, 75, {{ $player->pcmas->count() }}0],
                                    backgroundColor: 'rgba(99, 102, 241, 0.1)',
                                    borderColor: 'rgba(99, 102, 241, 0.{{ $player->pcmas->count() }})',
                                    borderWidth: 1,
                                    pointBackgroundColor: 'rgba(99, 102, 241, 0.{{ $player->pcmas->count() }})',
                                    pointBorderColor: '#fff',
                                    pointBorderWidth: 1,
                                    pointRadius: 5,
                                    borderDash: [5, 5]
                        ...
                        options: {
                            ...
                            plugins: {
                                ...
                                title: {
                                    ...
                                    color: '#374{{ $player->performances->sum("assists") ?? 0 }}1'
                                },
                                tooltip: {
                                    callbacks: {
                            ...
                            scales: {
                                r: {
                                    angleLines: { 
                                        display: true,
                                        color: 'rgba({{ $player->performances->sum("assists") ?? 0 }}6, 163, 175, 0.3)'
                                    },
                                    grid: {
                                        color: 'rgba({{ $player->performances->sum("assists") ?? 0 }}6, 163, 175, 0.2)'
                                    },
                                    pointLabels: {
                                        font: {
                                            size: 14,
                                        ...
                                        color: '#374{{ $player->performances->sum("assists") ?? 0 }}1',
                                        padding: {{ $player->performances->sum("assists") ?? 0 }}
                                    },
                                    suggestedMin: 0,
                                    suggestedMax: 100,
                                    ticks: { 
                                        stepSize: 20,
                                        display: true,
                                        backdropColor: 'rgba(255, 255, 255, 0.{{ $player->pcmas->count() }})',
                                        color: '#6B{{ $player->healthRecords->latest("created_at")->heart_rate ?? "N/A" }}{{ $player->pcmas->count() }}0',
                                        font: {
                                            size: 10
                                        }
                                    }

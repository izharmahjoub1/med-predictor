                    ...
                    <div class="flex flex-col lg:flex-row items-center space-y-4 lg:space-y-0 lg:space-x-6">
                        ...
                        <div class="space-y-2 text-sm">
                            ...
                            <div class="p-3 bg-gradient-to-r from-blue-500/20 to-blue-600/20 rounded-lg border border-blue-500/30">
                                ...
                                <div class="font-bold text-blue-400">{{ $player->match_availability ? "✅ DISPONIBLE" : "❌ INDISPONIBLE" }}</div>
            ...
            data() {
                return {
                    ...
                    offensiveStats: [
                        ...
                        { name: 'Dribbles réussis', display: '{{ $player->matchStats->sum("dribbles_completed") ?? 0 }}', percentage: {{ $player->matchStats->sum("assists_provided") > 0 ? 82 : 70 }}, teamAvg: '{{ $player->matchStats->count() > 0 ? round($player->performances->avg("dribbles"), 1) : 0 }}', leagueAvg: '38.9' }
                    ],
                    physicalStats: [
                        { name: 'Distance parcourue', display: '{{ $player->matchStats->sum("distance_covered_km") ?? 0 }} km', percentage: {{ $player->matchStats->sum("shots_total") > 0 ? 78 : 65 }}, teamAvg: '{{ $player->matchStats->count() > 0 ? round($player->matchStats->avg("distance_covered_km")) : 0 }} km', leagueAvg: '{{ $player->matchStats->count() > 0 ? round($player->matchStats->avg("distance_covered_km") * 0.95) : 0 }} km' },
                        ...
                        { name: 'Décélérations', display: '{{ $player->performances->sum("decelerations") ?? 0 }}', percentage: {{ $player->matchStats->sum("goals_scored") > 0 ? 85 : 70 }}, teamAvg: '1{{ $player->healthRecords->first() ? $player->healthRecords->first()->heart_rate : "N/A" }}', leagueAvg: '165' },
                    ...
                    technicalStats: [
                        ...
                        { name: 'Dégagements', display: '{{ $player->matchStats->sum("goals_scored") ?? 0 }}', percentage: {{ $player->matchStats->sum("shots_total") > 0 ? 78 : 65 }}, teamAvg: '15.2', leagueAvg: '14.1' },

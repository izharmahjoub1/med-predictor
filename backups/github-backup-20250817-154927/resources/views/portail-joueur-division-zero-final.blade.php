        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            data() {
                return {
                    ...
                    offensiveStats: [
                        { name: 'Buts marqués', display: '{{ $player->performances->sum("goals") ?? 0 }}', percentage: {{ $player->performances->sum("goals") > 0 ? 85 : 70 }}, teamAvg: '{{ $player->performances->count() > 0 ? round($player->performances->avg("goals"), 1) : 0 }}', leagueAvg: '7.2' },
                        ...
                        { name: 'Précision tirs', display: '{{ $player->performances->count() > 0 ? round(($player->performances->sum("shots_on_target") / $player->performances->sum("shots")) * 100) : 0 }}%', percentage: {{ $player->performances->sum("shots") > 0 ? 78 : 65 }}, teamAvg: '65%', leagueAvg: '62%' },
                        { name: 'Passes décisives', display: '{{ $player->performances->sum("assists") ?? 0 }}', percentage: {{ $player->performances->sum("assists") > 0 ? 82 : 70 }}, teamAvg: '{{ $player->performances->count() > 0 ? round($player->performances->avg("assists"), 1) : 0 }}', leagueAvg: '5.9' },
                        ...
                        { name: 'Centres réussis', display: '{{ $player->performances->sum("crosses") ?? 0 }}', percentage: 75, teamAvg: '{{ $player->performances->count() > 0 ? round($player->performances->avg("crosses"), 1) : 0 }}', leagueAvg: '13.7' },
                    ...
                    physicalStats: [
                        ...
                        { name: 'Sprints', display: '{{ $player->performances->sum("sprints") ?? 0 }}', percentage: {{ $player->performances->sum("assists") > 0 ? 82 : 70 }}, teamAvg: '{{ $player->performances->count() > 0 ? round($player->performances->avg("sprints")) : 0 }}', leagueAvg: '115' },

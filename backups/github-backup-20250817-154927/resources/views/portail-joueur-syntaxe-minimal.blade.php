<!{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}OCTYPE html>
<html lang="fr">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>{{ $player->first_name }} {{ $player->last_name }} - FIFA Ultimate {{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}ashboard</title>
    ...
    <style>
        ...
        .fifa-ultimate-card {
            background: linear-gradient(135deg, #1a2{{ $player->date_of_birth ? \Carbon\Carbon::parse($player->date_of_birth)->age : "N/A" }}e 0%, #303f9f 25%, #3f51b5 50%, #5c6bc0 75%, #9c27b0 100%);
            position: relative;
            overflow: hidden;
            border-radius: 24px;
...
<body class="bg-gray-50 min-h-screen font-sans">
                        ...
                        <div class="relative flex-shrink-0">
                            <div class="w-32 h-32 lg:w-36 lg:h-36 rounded-full overflow-hidden border-4 border-white/20 shadow-2xl bg-gradient-to-br from-gray-400 to-gray-600">
                                ...
                                <img src="https://cdn.futbin.com/content/fifa23/img/players/158023.png" 
                                     ...
                                     onerror="this.onerror=null; this.src='https://www.ea.com/fifa/ultimate-team/web-app/content/24B23F{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}E-7835-41C2-87A2-F3{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}F{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}B2E82/2024/fut/items/images/mobile/portraits/158023.png'; if(this.src.includes('ea.com') && this.complete && this.naturalHeight == 0) { this.style.display='none'; this.nextElementSibling.style.display='flex'; }">
                        ...
                        <div class="text-center lg:text-left">
                            ...
                            <div class="flex items-center justify-center lg:justify-start space-x-4 mb-4">
                                ...
                                <div class="flex items-center space-x-2">
                                    <img src="{{ $player->club->logo_url ?? $player->club->logo_path ?? "/images/clubs/default_club.svg" }}" 
                                         ...
                                         onerror="this.onerror=null; this.src='data:image/svg+xml;base64,PHN2ZyB3a{{ $player->matchStats->where("match_result", "win")->count() > 0 ? "W" : "L" }}R0a{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}0iMzIiIGhla{{ $player->matchStats->where("match_result", "win")->count() > 0 ? "W" : "L" }}dod{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}0iMzIiIHZpZXdCb3g9IjAgMCAzMiAzMiIgZmlsb{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}0ibm9uZSIgeG1sbnM9Imh0dHA6Ly93d3cudzMub3JnLzIwM{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}Avc3ZnIj4KPGNpcmNsZSBje{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}0iMTYiIGN5PSIxNiIgcj0iMTYiIGZpbGw9IiMzYjgyZjYiLz4KPHN2ZyB4PSI4IiB5PSI4IiB3a{{ $player->matchStats->where("match_result", "win")->count() > 0 ? "W" : "L" }}R0a{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}0iMTYiIGhla{{ $player->matchStats->where("match_result", "win")->count() > 0 ? "W" : "L" }}dod{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}0iMTYiIGZpbGw9IndoaXRlIj4KPHN2ZyB2a{{ $player->matchStats->where("match_result", "win")->count() > 0 ? "W" : "L" }}V3Qm{{ $player->overall_rating ?? "N/A" }}PSIwI{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}AgMjQgMjQiPgo8cGF0aCBkPSJNMTIgMkMxMy4xI{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}IgMTQgMi{{ $player->matchStats->count() }}I{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}E0I{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}RIMFY2S{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}E2VjEwS{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}EwVjE0S{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}E2VjE2S{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}EwVjIwQzE0I{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}IxLjEgMTMuMSAyMiAxMiAyMkMxMC45I{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}IyI{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}EwI{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}IxLjEgMTAgMjB{{ $player->matchStats->where("match_result", "win")->count() > 0 ? "W" : "L" }}MjBIMFYxOEgxMFYxNEg4VjEwS{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}EwVjR{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}MTAgMi45I{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}EwLjkgMiAxMiAy{{ $player->matchStats->where("match_result", "win")->count() > 0 ? "W" : "L" }}iIvPgo8L3N2Zz4KPC9zdmc+Cjwvc3ZnPgo='; if(this.complete && this.naturalHeight == 0) { this.style.display='none'; this.nextElementSibling.style.display='inline-block'; }">
                                    <!-- Fallback club icon -->
                                ...
                                <div class="flex items-center space-x-2">
                                    <img src="https://flagcdn.com/w40/ar.png" 
                                         ...
                                         onerror="this.onerror=null; this.src='data:image/svg+xml;base64,PHN2ZyB3a{{ $player->matchStats->where("match_result", "win")->count() > 0 ? "W" : "L" }}R0a{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}0iMjQiIGhla{{ $player->matchStats->where("match_result", "win")->count() > 0 ? "W" : "L" }}dod{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}0iMTYiIHZpZXdCb3g9IjAgMCAyNCAxNiIgZmlsb{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}0ibm9uZSIgeG1sbnM9Imh0dHA6Ly93d3cudzMub3JnLzIwM{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}Avc3ZnIj4KPHJlY3Qgd2lkdGg9IjI0IiBoZ{{ $player->matchStats->where("match_result", "win")->count() > 0 ? "W" : "L" }}lnaHQ9IjE2IiBma{{ $player->matchStats->where("match_result", "win")->count() > 0 ? "W" : "L" }}xsPSIjNGY0NmU1Ii8+Cjx0ZXh0IHg9IjEyIiB5PSIxMCIgdGV4dC1hbmNob3I9Im1pZGRsZSIgZmlsb{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}0id2hpdGUiIGZvbnQtZmFta{{ $player->matchStats->where("match_result", "win")->count() > 0 ? "W" : "L" }}x5PSJBcmlhbCIgZm9udC1zaXplPSIxMiI+wqA8L3RleHQ+Cjwvc3ZnPgo='; if(this.complete && this.naturalHeight == 0) { this.style.display='none'; this.nextElementSibling.style.display='inline-block'; }">
                                    <!-- Fallback nation -->
                            ...
                            <div class="grid grid-cols-2 md:grid-cols-2 gap-3 text-sm mb-4">
                                <div class="text-center p-2 bg-white/10 rounded-lg">
                                    <div class="font-semibold text-yellow-400">{{ $player->date_of_birth ? \Carbon\Carbon::parse($player->date_of_birth)->age : "N/A" }}</div>
                            ...
                            <div class="grid grid-cols-3 gap-2 text-center text-xs">
                                <div class="p-2 bg-white/5 rounded">
                                    <div class="font-bold text-green-400">{{ $player->matchStats->sum("goals_scored") ?? 0 }}</div>
                                    <div class="opacity-70">Buts saison</div>
                                </div>
                                <div class="p-2 bg-white/5 rounded">
                                    <div class="font-bold text-blue-400">{{ $player->matchStats->sum("assists_provided") ?? 0 }}</div>
                                    <div class="opacity-70">Assists</div>
                                </div>
                                <div class="p-2 bg-white/5 rounded">
                                    <div class="font-bold text-purple-400">{{ $player->matchStats->sum("goals_scored") ?? 0 }}</div>
                    ...
                    <div class="flex flex-col lg:flex-row items-center space-y-4 lg:space-y-0 lg:space-x-6">
                        ...
                        <div class="text-center">
                            <div class="flex space-x-3 mb-2">
                                <div class="fifa-rating-badge text-black p-4">
                                    <div class="text-3xl font-bold">{{ $player->overall_rating ?? "N/A" }}</div>
                                    <div class="text-xs">OVR</div>
                                </div>
                                <div class="fifa-rating-badge text-black p-3">
                                    <div class="text-2xl font-bold">{{ $player->potential_rating ?? "N/A" }}</div>
                            ...
                            <div class="relative w-20 h-20 mb-2">
                                <svg class="w-full h-full progress-circle" viewBox="0 0 100 100">
                                    ...
                                    <circle cx="50" cy="50" r="40" stroke="#10b981" stroke-width="8" fill="none"
                                            stroke-dasharray="251.33" stroke-dashoffset="{{ $player->date_of_birth ? \Carbon\Carbon::parse($player->date_of_birth)->age : "N/A" }}.7" stroke-linecap="round"/>
                                </svg>
                                <div class="absolute inset-0 flex items-center justify-center">
                                    <span class="text-xl font-bold text-green-400">{{ $player->ghs_overall_score ?? "N/A" }}</span>
                                </div>
                            </div>
                        ...
                        <div class="space-y-2 text-sm">
                            <div class="p-3 bg-gradient-to-r from-green-500/20 to-green-600/20 rounded-lg border border-green-500/30">
                                ...
                                <div class="font-bold text-green-400">{{ $player->injury_risk_score ?? 15 }}% - {{ $player->injury_risk_level ?? "FAIBLE" }}</div>
                            ...
                            <div class="p-3 bg-gradient-to-r from-yellow-500/20 to-yellow-600/20 rounded-lg border border-yellow-500/30">
                                ...
                                <div class="font-bold text-yellow-400">€{{ number_format(($player->value_eur ?? 180000000) / 1000000, 0) }}M</div>
                                <div class="text-xs text-yellow-300">↗️ +€{{ number_format((($player->value_eur ?? 180000000) - (($player->value_eur ?? 180000000) * 0.9)) / 1000000, 0) }}M ce mois</div>
                            </div>
                            
                            <div class="p-3 bg-gradient-to-r from-blue-500/20 to-blue-600/20 rounded-lg border border-blue-500/30">
                                <div class="text-xs text-blue-300 font-medium">{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}isponibilité</div>
                                <div class="font-bold text-blue-400">{{ $player->match_availability ? "✅ {{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}ISPONIBLE" : "❌ IN{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}ISPONIBLE" }}</div>
                                <div class="text-xs text-blue-300">Prochain match: {{ $player->next_match_date ?? "Non programmé" }}</div>
                            </div>

                            <!-- Indicateurs additionnels -->
                            <div class="grid grid-cols-2 gap-1 text-xs">
                                <div class="text-center p-2 bg-white/5 rounded">
                                    <div class="font-bold text-orange-400">{{ $player->ghs_physical_score ?? 85 }}%</div>
                                    <div class="opacity-70">Forme</div>
                                </div>
                                <div class="text-center p-2 bg-white/5 rounded">
                                    <div class="font-bold text-cyan-400">{{ $player->ghs_overall_score ?? 88 }}%</div>
                ...
                <div class="mt-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div class="lg:col-span-2 p-4 bg-white/5 rounded-xl backdrop-blur-sm">
                        <div class="flex justify-between items-center mb-2">
                            <span class="text-sm font-medium">Saison {{ date("Y") }}-{{ date("Y") + 1 }}</span>
                            <span class="text-sm text-white/80">{{ $player->matchStats->count() > 0 ? round(($player->matchStats->count() / ($player->matchStats->count() + 10)) * 100) : 0 }}% complétée</span>
                        </div>
                        <div class="w-full bg-white/20 rounded-full h-2 mb-2">
                            <div class="bg-gradient-to-r from-blue-500 to-purple-600 h-2 rounded-full" style="width: 75%"></div>
                        </div>
                        <div class="flex justify-between text-xs text-white/70">
                            <span>{{ $player->matchStats->sum("goals_scored") ?? 0 }} matchs joués</span>
                            <span>{{ max(0, 13 - ($player->matchStats->count() ?? 0)) }} matchs restants</span>
                        </div>
                    </div>
                    ...
                    <div class="p-4 bg-white/5 rounded-xl backdrop-blur-sm">
                        ...
                        <div class="flex justify-between items-center text-xs">
                            <span>{{ min(5, $player->matchStats->count()) }} derniers matchs:</span>
                            <div class="flex space-x-1">
                                <div class="w-4 h-4 bg-green-500 rounded-full flex items-center justify-center text-white text-xs">{{ $player->matchStats->where("match_result", "win")->count() > 0 ? "W" : "L" }}</div>
                                <div class="w-4 h-4 bg-green-500 rounded-full flex items-center justify-center text-white text-xs">{{ $player->matchStats->where("match_result", "win")->count() > 0 ? "W" : "L" }}</div>
                                <div class="w-4 h-4 bg-yellow-500 rounded-full flex items-center justify-center text-white text-xs">{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}</div>
                                <div class="w-4 h-4 bg-green-500 rounded-full flex items-center justify-center text-white text-xs">{{ $player->matchStats->where("match_result", "win")->count() > 0 ? "W" : "L" }}</div>
                                <div class="w-4 h-4 bg-green-500 rounded-full flex items-center justify-center text-white text-xs">{{ $player->matchStats->where("match_result", "win")->count() > 0 ? "W" : "L" }}</div>
                            </div>
                        </div>
        ...
        <div class="px-6 pb-6">
            ...
            <div v-show="activeTab === 'performance'" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div class="performance-card p-6 text-center">
                        ...
                        <div class="text-sm text-gray-600">{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}istance parcourue</div>
                    ...
                    <div class="performance-card p-6">
                        ...
                        <div class="space-y-3">
                            ...
                            <div class="flex justify-between items-center p-3 bg-green-50 rounded-lg">
                                ...
                                <div class="text-2xl font-bold text-green-600">{{ $player->matchStats->sum("distance_covered_km") ?? 0 }} km</div>
                            </div>
                    ...
                    <div class="performance-card p-6">
                        ...
                        <div class="grid grid-cols-2 gap-3 text-sm">
                            ...
                            <div class="text-center p-2 bg-orange-50 rounded">
                                <div class="font-bold text-orange-600">{{ $player->matchStats->count() }}%</div>
                            ...
                            <div class="text-center p-2 bg-green-50 rounded">
                                ...
                                <div class="text-gray-600">{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}ribbles réussis</div>
                            </div>
                        </div>
                    </div>
                </div>
                        ...
                        <table class="w-full text-sm">
                            <thead>
                                <tr class="border-b">
                                    ...
                                    <th class="text-center py-2">{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}ernière perf.</th>
                                </tr>
                            </thead>
                            <tbody>
                                <tr class="border-b hover:bg-gray-50">
                                    ...
                                    <td class="text-center">{{ $player->matchStats->count() > 0 ? round($player->matchStats->avg("match_rating"), 1) : 0 }}</td>
            ...
            <div v-show="activeTab === 'notifications'" class="space-y-6">
                ...
                <div v-show="activeNotificationFilter === 'all' || activeNotificationFilter === 'national'" class="performance-card p-6">
                    ...
                    <div class="space-y-4">
                        <div v-for="notification in notification{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}ata.nationalTeam" :key="notification.id" 
                ...
                <div v-show="activeNotificationFilter === 'all' || activeNotificationFilter === 'training'" class="performance-card p-6">
                    ...
                    <div class="space-y-4">
                        <div v-for="session in notification{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}ata.trainingSessions" :key="session.id" 
                ...
                <div v-show="activeNotificationFilter === 'all' || activeNotificationFilter === 'matches'" class="performance-card p-6">
                    ...
                    <div class="space-y-4">
                        <div v-for="match in notification{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}ata.matches" :key="match.id" 
                            ...
                            <div class="flex-1">
                                ...
                                <div class="flex items-center space-x-4 text-sm">
                                    ...
                                    <span class="text-gray-500" v-text="'R{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}V: ' + match.meetingTime"></span>
                                </div>
                            </div>
                            <div class="flex-shrink-0">
                ...
                <div v-show="activeNotificationFilter === 'all' || activeNotificationFilter === 'medical'" class="performance-card p-6">
                    ...
                    <div class="space-y-4">
                        <div v-for="appointment in notification{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}ata.medicalAppointments" :key="appointment.id" 
                            ...
                            <div class="flex-1">
                                ...
                                <div class="text-sm text-gray-600 mt-1" v-text="'{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}r. ' + appointment.doctor"></div>
                            </div>
                ...
                <div v-show="activeNotificationFilter === 'all' || activeNotificationFilter === 'social'" class="performance-card p-6">
                    ...
                    <div class="space-y-4">
                        <div v-for="alert in notification{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}ata.socialAlerts" :key="alert.id" 
            ...
            <div v-show="activeTab === 'health'" class="space-y-6">
                ...
                <div class="grid grid-cols-2 md:grid-cols-2 gap-4">
                    <div class="performance-card p-4 text-center">
                        <div class="text-3xl font-bold text-green-600" v-text="health{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}ata.globalScore"></div>
                        <div class="text-sm text-gray-600">Score FIT Global</div>
                        <div class="text-xs text-green-600 mt-1">↗️ +2 points</div>
                    </div>
                    <div class="performance-card p-4 text-center">
                        <div class="text-3xl font-bold text-blue-600" v-text="health{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}ata.physicalScore + '%'"></div>
                        <div class="text-sm text-gray-600">Condition Physique</div>
                        <div class="text-xs text-blue-600 mt-1">↗️ +3%</div>
                    </div>
                    <div class="performance-card p-4 text-center">
                        <div class="text-3xl font-bold text-purple-600" v-text="health{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}ata.sleepScore + '%'"></div>
                        <div class="text-sm text-gray-600">Qualité Sommeil</div>
                        <div class="text-xs text-purple-600 mt-1">↗️ +5%</div>
                    </div>
                    <div class="performance-card p-4 text-center">
                        <div class="text-3xl font-bold text-orange-600" v-text="health{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}ata.injuryRisk + '%'"></div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="performance-card p-6">
                        ...
                        <div class="space-y-4">
                            <div v-for="vital in health{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}ata.vitals" :key="vital.name" class="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="space-y-2">
                                ...
                                <div class="flex justify-between items-center p-2 bg-yellow-50 rounded">
                                    ...
                                    <span class="font-bold text-yellow-600">2{{ $player->matchStats->count() }}g / {{ $player->matchStats->sum("goals_scored") ?? 0 }}0g</span>
                                </div>
                                ...
                                <div class="flex justify-between items-center p-2 bg-purple-50 rounded">
                                    ...
                                    <span class="font-bold text-purple-600">{{ $player->matchStats->sum("goals_scored") ?? 0 }}40 / 3200</span>
                                </div>
                            </div>
                        </div>
                    </div>

                    <!-- Sommeil {{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}étaillé -->
                    <div class="performance-card p-6">
                        <h4 class="text-lg font-bold text-gray-800 mb-4 flex items-center">
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="space-y-2">
                                ...
                                <div class="flex justify-between items-center">
                                    ...
                                    <span class="font-bold text-purple-600">1h {{ $player->matchStats->count() }}min (21%)</span>
                                </div>
                                <div class="flex justify-between items-center">
                    ...
                    <div class="performance-card p-6">
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="space-y-2">
                                ...
                                <div class="w-full bg-gray-200 rounded-full h-2">
                                    <div class="bg-green-500 h-2 rounded-full" style="width: {{ $player->ghs_overall_score ?? 88 }}%"></div>
                                </div>
                                <div class="flex justify-between items-center">
                                ...
                                <div class="w-full bg-gray-200 rounded-full h-2">
                                    <div class="bg-indigo-500 h-2 rounded-full" style="width: {{ $player->ghs_physical_score ?? 85 }}%"></div>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <!-- Facteurs S{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}OH (Social {{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}eterminants of Health) -->
                <div class="performance-card p-6 bg-white rounded-2xl shadow-lg">
                    <h4 class="text-xl font-bold text-gray-800 mb-4 flex items-center">
                        <i class="fas fa-users text-teal-600 mr-3"></i>
                        Santé & Bien-être – Facteurs S{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}OH
                    </h4>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <!-- Radar Chart S{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}OH -->
                        <div class="w-full">
                        ...
                        <!-- S{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}OH {{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}etails -->
                        <div class="w-full space-y-4">
                            <p class="text-gray-700 mb-4">
                                Cet indicateur <strong>S{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}OH</strong> (Social {{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}eterminants of Health) donne une vision
                            ...
                            <!-- S{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}OH Factors breakdown -->
                            <div class="space-y-3">
                            ...
                            <!-- Overall S{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}OH Score -->
                            <div class="mt-6 p-4 bg-gradient-to-r from-teal-50 to-green-50 rounded-lg border border-teal-200">
                                <div class="flex items-center justify-between">
                                    <div>
                                        <div class="font-bold text-gray-800">Score S{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}OH Global</div>
                                        <div class="text-sm text-gray-600">{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}éterminants sociaux de santé</div>
                                    </div>
                                    <div class="text-right">
                ...
                <div class="performance-card p-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div v-for="recommendation in health{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}ata.recommendations" :key="recommendation.id" 
                ...
                <!-- Objectifs et {{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}éfis -->
                <div class="performance-card p-6">
                    <h4 class="text-lg font-bold text-gray-800 mb-4 flex items-center">
                        <i class="fas fa-target text-red-600 mr-2"></i>
                        Objectifs & {{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}éfis de la Semaine
                    </h4>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div class="performance-card p-6">
                    <div class="flex justify-between items-center mb-6">
                        <h3 class="text-xl font-bold text-gray-800 flex items-center">
                            ...
                            {{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}ossier Médical Complet
                        </h3>
                        <div class="flex space-x-3">
                            ...
                            <button onclick="exportMedicalP{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}F()" class="px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 transition-all">
                                <i class="fas fa-file-pdf mr-2"></i>Export P{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}F
                            </button>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                        <div class="text-center p-4 bg-blue-50 rounded-lg">
                            <div class="text-2xl font-bold text-blue-600" v-text="medical{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}ata.healthRecords.length"></div>
                            <div class="text-sm text-gray-600">{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}ossiers de Santé</div>
                        </div>
                        <div class="text-center p-4 bg-green-50 rounded-lg">
                            <div class="text-2xl font-bold text-green-600" v-text="medical{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}ata.pcmas.length"></div>
                            <div class="text-sm text-gray-600">Évaluations PCMA</div>
                        </div>
                        <div class="text-center p-4 bg-purple-50 rounded-lg">
                            <div class="text-2xl font-bold text-purple-600" v-text="medical{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}ata.predictions.length"></div>
                            <div class="text-sm text-gray-600">Prédictions IA</div>
                        </div>
                    </div>

                    <!-- {{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}ossiers récents -->
                    <div class="space-y-3 mb-6">
                        <h4 class="font-bold text-gray-800">📋 {{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}ossiers Récents</h4>
                        <div v-for="record in medical{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}ata.healthRecords.slice(0, 3)" :key="record.id" 
                    ...
                    <div class="space-y-3 mb-6">
                        ...
                        <div v-for="injury in medical{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}ata.injuries" :key="injury.id" 
                    ...
                    <div class="space-y-3">
                        ...
                        <div v-for="illness in medical{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}ata.illnesses" :key="illness.id" 
                             class="p-4 border rounded-lg">
                            <div class="flex justify-between items-start mb-2">
                                <div>
                    ...
                    <div class="performance-card p-6 mt-6">
                        ...
                        <div class="space-y-4">
                            <div v-for="injury in medical{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}ata.injuries" :key="injury.id" class="border rounded-lg p-4">
                                <div class="flex justify-between items-start mb-2">
                                    <div>
                                ...
                                <div class="text-xs text-gray-500">
                                    ...
                                    <span>{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}urée estimée: </span><span v-text="injury.estimated_duration"></span>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="performance-card p-6 mt-6">
                        ...
                        <div class="space-y-4">
                            <div v-for="illness in medical{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}ata.illnesses" :key="illness.id" class="border rounded-lg p-4">
                                <div class="flex justify-between items-start mb-2">
                                    <div>
                                        <h5 class="font-bold text-gray-800" v-text="illness.name"></h5>
                                        <p class="text-sm text-gray-600" v-text="illness.date_diagnosed + ' • {{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}r. ' + illness.doctor"></p>
                                    </div>
                                    <div class="flex items-center space-x-2">
                                        <span :class="['px-2 py-1 rounded-full text-xs font-bold',
                                ...
                                <div class="text-xs text-gray-500">
                                    <span>{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}ernière consultation: </span><span v-text="illness.last_checkup"></span>
                                    <span v-if="illness.next_appointment"> • Prochain R{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}V: </span><span v-if="illness.next_appointment" v-text="illness.next_appointment"></span>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <!-- Onglet {{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}evices -->
            <div v-show="activeTab === 'devices'" class="space-y-6">
                <div class="performance-card p-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                        <div class="text-center p-4 bg-green-50 rounded-lg">
                            <div class="text-2xl font-bold text-green-600" v-text="device{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}ata.connected{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}evices"></div>
                            <div class="text-sm text-gray-600">Appareils connectés</div>
                        </div>
                        <div class="text-center p-4 bg-blue-50 rounded-lg">
                            <div class="text-2xl font-bold text-blue-600" v-text="device{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}ata.daily{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}ataPoints"></div>
                            <div class="text-sm text-gray-600">Points de données/jour</div>
                        </div>
                        <div class="text-center p-4 bg-purple-50 rounded-lg">
                            <div class="text-2xl font-bold text-purple-600" v-text="device{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}ata.lastSync"></div>
                            <div class="text-sm text-gray-600">{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}ernière sync</div>
                        </div>
                    </div>

                    <!-- Liste des devices -->
                    <div class="space-y-4">
                        <h4 class="font-bold text-gray-800">📱 Appareils {{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}isponibles</h4>
                        <div v-for="device in device{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}ata.devices" :key="device.id" 
                            ...
                            <div class="flex items-center space-x-4">
                                ...
                                <div>
                                    ...
                                    <div class="text-xs text-gray-500" v-text="'{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}ernières données: ' + device.last{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}ata"></div>
                                </div>
                            </div>
                            <div class="text-right">
                                <div :class="['px-3 py-1 rounded-full text-xs font-bold', device.connected ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800']" 
                                     v-text="device.connected ? 'CONNECTÉ' : '{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}ÉCONNECTÉ'"></div>
                                <div v-if="device.batteryLevel" class="text-xs text-gray-500 mt-1" v-text="'Batterie: ' + device.batteryLevel + '%'"></div>
                            </div>
                        </div>
                    </div>

                    <!-- {{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}onnées récentes -->
                    <div class="mt-6">
                        <h4 class="font-bold text-gray-800 mb-3">📊 {{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}onnées Récentes (24h)</h4>
                        <div class="grid grid-cols-2 md:grid-cols-2 gap-4">
                            <div class="text-center p-3 bg-red-50 rounded-lg">
                                <div class="text-lg font-bold text-red-600" v-text="device{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}ata.recent{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}ata.heartRate"></div>
                                <div class="text-xs text-gray-600">FC Moyenne (bpm)</div>
                            </div>
                            <div class="text-center p-3 bg-blue-50 rounded-lg">
                                <div class="text-lg font-bold text-blue-600" v-text="device{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}ata.recent{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}ata.steps"></div>
                                <div class="text-xs text-gray-600">Pas</div>
                            </div>
                            <div class="text-center p-3 bg-green-50 rounded-lg">
                                <div class="text-lg font-bold text-green-600" v-text="device{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}ata.recent{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}ata.sleep"></div>
                                <div class="text-xs text-gray-600">Sommeil (h)</div>
                            </div>
                            <div class="text-center p-3 bg-purple-50 rounded-lg">
                                <div class="text-lg font-bold text-purple-600" v-text="device{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}ata.recent{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}ata.calories"></div>
            ...
            <!-- Onglet {{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}opage (complet) -->
            <div v-show="activeTab === 'doping'" class="space-y-6">
                ...
                <div class="performance-card p-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="text-center p-6 bg-green-50 rounded-lg">
                            <div class="text-lg font-bold text-green-600" v-text="doping{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}ata.lastControl"></div>
                            <div class="text-sm text-gray-600">{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}ernier contrôle</div>
                            <div class="text-xs text-green-600 mt-2">✅ NÉGATIF</div>
                        </div>
                        <div class="text-center p-6 bg-blue-50 rounded-lg">
                            <div class="text-lg font-bold text-blue-600" v-text="doping{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}ata.nextControl"></div>
                ...
                <div class="performance-card p-6">
                    ...
                    <div class="space-y-3">
                        <div v-for="control in doping{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}ata.controlHistory" :key="control.id" 
                             class="p-4 border rounded-lg">
                            <div class="flex justify-between items-start mb-2">
                                <div>
                ...
                <!-- {{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}éclarations ATU -->
                <div class="performance-card p-6">
                    <h4 class="text-lg font-bold text-gray-800 mb-4">💊 {{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}éclarations ATU Actives</h4>
                    <div class="space-y-3">
                        <div v-for="atu in doping{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}ata.atu{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}eclarations" :key="atu.id" 
                             class="p-4 border rounded-lg">
                            <div class="flex justify-between items-start mb-2">
                                <div>
                            ...
                            <div class="text-sm text-gray-700 mb-2">
                                <span class="font-medium">{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}osage:</span> <span v-text="atu.dosage"></span><br>
                                <span class="font-medium">Fréquence:</span> <span v-text="atu.frequency"></span><br>
                                <span class="font-medium">Période:</span> <span v-text="atu.start{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}ate + ' - ' + atu.end{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}ate"></span>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="performance-card p-6">
                        ...
                        <div class="space-y-3">
                            <div v-for="alert in doping{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}ata.riskAlerts" :key="alert.id" 
                    ...
                    <div class="performance-card p-6">
                        ...
                        <div class="space-y-2">
                            <div v-for="substance in doping{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}ata.prohibitedSubstances" :key="substance" 
    ...
    <script>
        ...
        document.addEventListener('{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}OMContentLoaded', function() {
            console.log('📱 {{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}OM chargé, initialisation Vue.js...');
            ...
            data() {
                return {
                    ...
                    navigationTabs: [
                        ...
                        { id: 'devices', name: '{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}evices', icon: 'fas fa-mobile-alt', count: 3 },
                        { id: 'doping', name: '{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}opage', icon: 'fas fa-vial', count: null }
                    ],
                    notificationFilters: [
                    ...
                    offensiveStats: [
                        { name: 'Buts marqués', display: '{{ $player->matchStats->sum("goals_scored") ?? 0 }}', percentage: {{ $player->matchStats->sum("goals_scored") > 0 ? 85 : 70 }}, teamAvg: '{{ $player->matchStats->count() > 0 ? $player->matchStats->count() > 0 ? round($player->performances->avg("goals"), 1) : 0 : 0 }}', leagueAvg: '7.2' },
                        { name: 'Tirs cadrés', display: '{{ $player->matchStats->sum("shots_on_target") ?? 0 }}', percentage: {{ $player->matchStats->sum("shots_total") > 0 ? 78 : 65 }}, teamAvg: '{{ $player->matchStats->count() > 0 ? round($player->matchStats->avg("shots_on_target"), 1) : 0 }}', leagueAvg: '{{ $player->matchStats->count() > 0 ? round($player->matchStats->avg("shots_on_target") * 1.1, 1) : 0 }}' },
                        { name: 'Tirs totaux', display: '{{ $player->matchStats->sum("shots_total") ?? 0 }}', percentage: 72, teamAvg: '{{ $player->matchStats->count() > 0 ? round($player->matchStats->avg("shots_total"), 1) : 0 }}', leagueAvg: '{{ $player->matchStats->count() > 0 ? round($player->matchStats->avg("shots_total") * 1.1, 1) : 0 }}' },
                        { name: 'Précision tirs', display: '{{ $player->matchStats->sum("shots_total") > 0 ? round(($player->matchStats->sum("shots_on_target") / $player->matchStats->sum("shots_total")) * 100) : 0 }}%', percentage: {{ $player->matchStats->sum("shots_total") > 0 ? 78 : 65 }}, teamAvg: '{{ $player->matchStats->sum("shots_total") > 0 ? round(($player->matchStats->sum("shots_on_target") / $player->matchStats->sum("shots_total")) * 100) : 0 }}%', leagueAvg: '{{ $player->matchStats->sum("shots_total") > 0 ? round(($player->matchStats->sum("shots_on_target") / $player->matchStats->sum("shots_total")) * 100 * 0.95) : 0 }}%' },
                        { name: 'Passes décisives', display: '{{ $player->matchStats->sum("assists_provided") ?? 0 }}', percentage: {{ $player->matchStats->sum("assists_provided") > 0 ? 82 : 70 }}, teamAvg: '{{ $player->matchStats->count() > 0 ? $player->matchStats->count() > 0 ? round($player->performances->avg("assists"), 1) : 0 : 0 }}', leagueAvg: '5.9' },
                        { name: 'Passes clés', display: '{{ $player->matchStats->sum("key_passes") ?? 0 }}', percentage: {{ $player->matchStats->sum("shots_total") > 0 ? 78 : 65 }}, teamAvg: '{{ $player->matchStats->count() > 0 ? round($player->matchStats->avg("key_passes"), 1) : 0 }}', leagueAvg: '{{ $player->matchStats->count() > 0 ? round($player->matchStats->avg("key_passes") * 1.1, 1) : 0 }}' },
                        { name: 'Centres réussis', display: '{{ $player->matchStats->sum("crosses_total") ?? 0 }}', percentage: {{ $player->matchStats->sum("crosses_total") > 0 ? 75 : 65 }}, teamAvg: '{{ $player->matchStats->count() > 0 ? $player->matchStats->count() > 0 ? round($player->performances->avg("crosses"), 1) : 0 : 0 }}', leagueAvg: '13.7' },
                        { name: '{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}ribbles réussis', display: '{{ $player->matchStats->sum("dribbles_completed") ?? 0 }}', percentage: {{ $player->matchStats->sum("assists_provided") > 0 ? 82 : 70 }}, teamAvg: '{{ $player->matchStats->count() > 0 ? round($player->performances->avg("dribbles"), 1) : 0 }}', leagueAvg: '38.9' }
                    ],
                    physicalStats: [
                        { name: '{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}istance parcourue', display: '{{ $player->matchStats->sum("distance_covered_km") ?? 0 }} km', percentage: {{ $player->matchStats->sum("shots_total") > 0 ? 78 : 65 }}, teamAvg: '{{ $player->matchStats->count() > 0 ? round($player->matchStats->avg("distance_covered_km")) : 0 }} km', leagueAvg: '{{ $player->matchStats->count() > 0 ? round($player->matchStats->avg("distance_covered_km") * 0.95) : 0 }} km' },
                        { name: 'Vitesse maximale', display: '{{ $player->matchStats->max("max_speed_kmh") ?? 0 }} km/h', percentage: {{ $player->matchStats->sum("goals_scored") > 0 ? 85 : 70 }}, teamAvg: '{{ $player->matchStats->count() > 0 ? round($player->matchStats->avg("max_speed_kmh"), 1) : 0 }} km/h', leagueAvg: '{{ $player->matchStats->count() > 0 ? round($player->matchStats->avg("max_speed_kmh") * 0.95, 1) : 0 }} km/h' },
                        { name: 'Vitesse moyenne', display: '{{ $player->matchStats->avg("avg_speed_kmh") ?? 0 }} km/h', percentage: {{ $player->matchStats->sum("assists_provided") > 0 ? 82 : 70 }}, teamAvg: '{{ $player->matchStats->count() > 0 ? round($player->matchStats->avg("avg_speed_kmh"), 1) : 0 }} km/h', leagueAvg: '{{ $player->matchStats->count() > 0 ? round($player->matchStats->avg("avg_speed_kmh") * 0.95, 1) : 0 }} km/h' },
                        { name: 'Sprints', display: '{{ $player->matchStats->sum("sprint_count") ?? 0 }}', percentage: {{ $player->matchStats->sum("assists_provided") > 0 ? 82 : 70 }}, teamAvg: '{{ $player->matchStats->count() > 0 ? $player->matchStats->count() > 0 ? round($player->matchStats->avg("sprint_count")) : 0 : 0 }}', leagueAvg: '115' },
                        { name: 'Accélérations', display: '{{ $player->matchStats->sum("acceleration_count") ?? 0 }}', percentage: {{ $player->matchStats->sum("acceleration_count") > 0 ? 87 : 75 }}, teamAvg: '{{ $player->matchStats->count() > 0 ? round($player->matchStats->avg("acceleration_count")) : 0 }}', leagueAvg: '{{ $player->matchStats->count() > 0 ? round($player->matchStats->avg("acceleration_count") * 0.95) : 0 }}' },
                        { name: '{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}écélérations', display: '{{ $player->performances->sum("decelerations") ?? 0 }}', percentage: {{ $player->matchStats->sum("goals_scored") > 0 ? 85 : 70 }}, teamAvg: '1{{ $player->healthRecords->first() ? $player->healthRecords->first()->heart_rate : "N/A" }}', leagueAvg: '165' },
                        { name: 'Changements direction', display: '{{ $player->matchStats->sum("direction_changes") ?? 0 }}', percentage: {{ $player->matchStats->sum("shots_total") > 0 ? 78 : 65 }}, teamAvg: '{{ $player->matchStats->sum("direction_changes") ?? 0 }}', leagueAvg: '{{ $player->healthRecords->first() ? $player->healthRecords->first()->heart_rate : "N/A" }}' },
                        { name: 'Sautes', display: '{{ $player->matchStats->sum("goals_scored") ?? 0 }}', percentage: 70, teamAvg: '15.2', leagueAvg: '14.8' }
                    ],
                    technicalStats: [
                        { name: 'Précision passes', display: '{{ $player->matchStats->sum("passes_total") > 0 ? round(($player->matchStats->sum("passes_completed") / $player->matchStats->sum("passes_total")) * 100) : 0 }}%', percentage: {{ $player->matchStats->sum("assists_provided") > 0 ? 82 : 70 }}, teamAvg: '{{ $player->matchStats->sum("passes_total") > 0 ? round(($player->matchStats->sum("passes_completed") / $player->matchStats->sum("passes_total")) * 100) : 0 }}%', leagueAvg: '{{ $player->matchStats->sum("passes_total") > 0 ? round(($player->matchStats->sum("passes_completed") / $player->matchStats->sum("passes_total")) * 100 * 0.95) : 0 }}%' },
                        { name: 'Passes longues', display: '{{ $player->matchStats->count() }}', percentage: {{ $player->matchStats->sum("shots_total") > 0 ? 78 : 65 }}, teamAvg: '38', leagueAvg: '35' },
                        { name: 'Centres réussis', display: '{{ $player->matchStats->sum("crosses_total") ?? 0 }}', percentage: {{ $player->matchStats->sum("crosses_total") > 0 ? 75 : 65 }}, teamAvg: '{{ $player->matchStats->count() > 0 ? $player->matchStats->count() > 0 ? round($player->performances->avg("crosses"), 1) : 0 : 0 }}', leagueAvg: '13.7' },
                        { name: 'Tacles réussis', display: '{{ $player->performances->sum("tackles") ?? 0 }}', percentage: {{ $player->matchStats->sum("assists_provided") > 0 ? 82 : 70 }}, teamAvg: '{{ $player->matchStats->sum("goals_scored") ?? 0 }}.5', leagueAvg: '26.8' },
                        { name: 'Interceptions', display: '{{ $player->performances->sum("interceptions") ?? 0 }}', percentage: {{ $player->matchStats->sum("goals_scored") > 0 ? 85 : 70 }}, teamAvg: '26.8', leagueAvg: '24.5' },
                        { name: '{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}égagements', display: '{{ $player->matchStats->sum("goals_scored") ?? 0 }}', percentage: {{ $player->matchStats->sum("shots_total") > 0 ? 78 : 65 }}, teamAvg: '15.2', leagueAvg: '14.1' },
                        { name: 'Fautes commises', display: '{{ $player->matchStats->sum("assists_provided") ?? 0 }}', percentage: {{ $player->matchStats->sum("assists_provided") > 0 ? 82 : 70 }}, teamAvg: '12.4', leagueAvg: '11.8' },
                        { name: 'Cartons jaunes', display: '{{ $player->matchStats->sum("yellow_cards") ?? 0 }}', percentage: {{ $player->matchStats->sum("goals_scored") > 0 ? 85 : 70 }}, teamAvg: '{{ $player->matchStats->count() > 0 ? round($player->matchStats->avg("yellow_cards"), 1) : 0 }}', leagueAvg: '{{ $player->matchStats->count() > 0 ? round($player->matchStats->avg("yellow_cards") * 1.1, 1) : 0 }}' }
                    ],
                    seasonSummary: {
                        'goals': {'total': 12, 'trend': '+3', 'avg': 0.43},
                        'assists': {'total': 8, 'trend': '+2', 'avg': 0.29},
                        'matches': {'total': {{ $player->matchStats->sum("goals_scored") ?? 0 }}, 'rating': {{ $player->matchStats->count() > 0 ? round($player->matchStats->avg("match_rating"), 1) : 0 }}, 'distance': '2{{ $player->matchStats->count() }} km'}
                    },
                    performanceEvolution: {
                        'labels': ['Jan', 'Fév', 'Mar', 'Avr', 'Mai', 'Juin'],
                        'ratings': [8.2, 8.4, 8.6, {{ $player->matchStats->count() > 0 ? round($player->matchStats->avg("match_rating"), 1) : 0 }}, 8.8, {{ $player->matchStats->count() > 0 ? round($player->matchStats->avg("match_rating"), 1) : 0 }}],
                        'goals': [2, 3, 2, 1, 2, 2],
                        'assists': [1, 2, 1, 1, 2, 1]
                    },
                    notification{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}ata: {
                        'nationalTeam': [
                        ...
                        'matches': [
                            { id: 6, title: 'Próximo Partido', message: '{{ $player->club->name ?? "Club" }} vs {{ $player->club->association->name ?? "Adversaire" }} - {{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}omingo 16/03', date: '10/03/2025', priority: 'high', type: 'matches', status: 'convoqué', icon: 'fas fa-futbol', competition: '{{ $player->club->association->name ?? "Compétition" }}', stadium: '{{ $player->club->stadium_name ?? "Stade" }}', time: '15:00', meetingTime: '14:00' },
                        ...
                        'medicalAppointments': [
                            { id: 8, title: 'Control Médico', message: 'Revisión rutinaria con {{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}r. Martínez', date: '08/03/2025', priority: 'medium', type: 'medical', urgent: false, icon: 'fas fa-user-md', type: 'Consultation', purpose: 'Revisión rutinaria', time: '10:00', location: 'Centre médical', doctor: '{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}r. Martínez' },
                        ...
                        'socialAlerts': [
                            ...
                            { id: 3, title: 'Instagram Live', content: 'Solicitud de entrevista en vivo', timestamp: '04/03/2025', views: '{{ $player->matchStats->count() > 0 ? round($player->matchStats->avg("match_rating"), 1) : 0 }}K', engagement: '{{ $player->matchStats->sum("shots_on_target") ?? 0 }}3', sentiment: 'neutral', platform: 'instagram', needsResponse: true }
                        ]
                    },
                    health{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}ata: {
                        'globalScore': 87,
                        'physicalScore': 82,
                        'mentalScore': 85,
                        'sleepScore': 86,
                    ...
                    device{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}ata: {
                        connected{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}evices: 3,
                        daily{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}ataPoints: 1440,
                        lastSync: '2 min',
                        devices: [
                            { id: 1, name: 'Polar H10', type: 'Capteur cardiaque', brand: 'Polar', icon: 'fas fa-heartbeat', connected: true, batteryLevel: 85, last{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}ata: 'Il y a 2 min' },
                            { id: 2, name: '{{ $player->matchStats->where("match_result", "win")->count() > 0 ? "W" : "L" }}HOOP 4.0', type: 'Tracker fitness', brand: '{{ $player->matchStats->where("match_result", "win")->count() > 0 ? "W" : "L" }}HOOP', icon: 'fas fa-running', connected: true, batteryLevel: 62, last{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}ata: 'Il y a 5 min' },
                            { id: 3, name: 'GPS Catapult', type: 'Tracker GPS', brand: 'Catapult', icon: 'fas fa-satellite', connected: true, batteryLevel: 88, last{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}ata: 'Il y a 1 min' }
                        ],
                        recent{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}ata: {
                            heartRate: 68,
                            steps: 15{{ $player->matchStats->sum("shots_on_target") ?? 0 }}0,
                            sleep: '7h32',
                            calories: {{ $player->matchStats->sum("goals_scored") ?? 0 }}47
                        }
                    },
                    doping{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}ata: {
                        lastControl: '{{ $player->matchStats->sum("goals_scored") ?? 0 }}/02/2025',
                        nextControl: '15/04/2025',
                        controlHistory: [
                            { id: 1, type: 'Contrôle inopiné', date: '{{ $player->matchStats->sum("goals_scored") ?? 0 }}/02/2025', location: 'Centre d\'entraînement', status: 'completed', result: 'négatif', substances: 'Aucune substance interdite détectée', notes: 'Contrôle standard, aucun problème détecté' },
                        ...
                        atu{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}eclarations: [
                            { id: 1, substance: 'Ventoline (Salbutamol)', reason: 'Asthme d\'effort', dosage: '100mcg par inhalation', frequency: 'Avant entraînement si nécessaire', doctor: '{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}r. Jean Martin', start{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}ate: '01/09/2024', end{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}ate: '31/08/2025', status: 'active', riskLevel: 'low' }
                        ]
                    },
                    sdoh{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}ata: {
                        environment: 85,
                        socialSupport: 85,
                        healthcareAccess: 75,
                        financialStatus: 80,
                        mental{{ $player->matchStats->where("match_result", "win")->count() > 0 ? "W" : "L" }}ellbeing: 78,
                        education: 88,
                        employment: 87,
                        community: 85
                    },
                    medical{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}ata: {
                        healthRecords: [
                            {
                                id: 1,
                                title: 'Consultation Cardiologique',
                                doctor: '{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}r. Jean Martin',
                                date: '05/03/2025',
                                status: 'completed'
                            },
                            {
                                ...
                                doctor: '{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}r. Sophie Moreau',
                                date: '{{ $player->matchStats->sum("goals_scored") ?? 0 }}/02/2025',
                                status: 'completed'
                            },
                            {
                                id: 3,
                            ...
                            },
                            {
                                ...
                                doctor: '{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}r. Pierre {{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}ubois',
                                date: '20/02/2025',
                                status: 'completed'
                            },
                            {
                                ...
                                doctor: '{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}r. Marie Laurent',
                                date: '15/02/2025',
                                status: 'completed'
                            },
                            {
                                id: 6,
                                title: 'Examen {{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}entaire',
                                doctor: '{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}r. Paul Bernard',
                                date: '10/02/2025',
                                status: 'completed'
                            }
                        ],
                        ...
                        injuries: [
                            ...
                            },
                            {
                                ...
                                severity: 'mild',
                                date: '{{ $player->matchStats->sum("goals_scored") ?? 0 }}/12/2024',
                                recovery: 100,
                                status: 'recovered',
                        ...
                        devices: [
                            {
                                ...
                                connected: true,
                                batteryLevel: 85,
                                last{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}ata: 'Il y a 2 min'
                            },
                            {
                                id: 2,
                                name: '{{ $player->matchStats->where("match_result", "win")->count() > 0 ? "W" : "L" }}HOOP 4.0',
                                type: 'Tracker fitness',
                                brand: '{{ $player->matchStats->where("match_result", "win")->count() > 0 ? "W" : "L" }}HOOP',
                                icon: 'fas fa-running',
                                connected: true,
                                batteryLevel: 62,
                                last{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}ata: 'Il y a 5 min'
                            },
                            {
                                id: 3,
                                name: 'GPS Catapult',
                                ...
                                connected: true,
                                batteryLevel: 88,
                                last{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}ata: 'Il y a 1 min'
                            },
                            {
                                id: 4,
                                name: 'Balance InBody',
                                ...
                                connected: false,
                                batteryLevel: null,
                                last{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}ata: 'Il y a 2h'
                            },
                            {
                                id: 5,
                                name: 'Oura Ring',
                                ...
                                icon: 'fas fa-bed',
                                connected: true,
                                batteryLevel: {{ $player->matchStats->count() }},
                                last{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}ata: 'Il y a 10 min'
                            }
                        ],
                        recent{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}ata: {
                            heartRate: 68,
                    ...
                    doping{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}ata: {
                        ...
                        controlHistory: [
                            ...
                            },
                            {
                                ...
                                type: 'Contrôle ciblé',
                                date: '30/01/2025',
                                location: '{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}omicile',
                                status: 'completed',
                                result: 'négatif',
                                substances: 'Aucune substance interdite détectée',
                        ...
                        atu{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}eclarations: [
                            {
                                ...
                                doctor: '{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}r. Jean Martin',
                                start{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}ate: '01/09/2024',
                                end{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}ate: '31/08/2025',
                                status: 'active',
                                riskLevel: 'low'
                            },
                            {
                                id: 2,
                                substance: 'Vitamine {{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}3',
                                reason: 'Carence saisonnière',
                                dosage: '2000 UI par jour',
                                frequency: '1 fois par jour',
                                doctor: '{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}r. Sophie Moreau',
                                start{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}ate: '01/12/2024',
                                end{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}ate: '31/03/2025',
                                status: 'active',
                                riskLevel: 'none'
                            }
                        ],
                        ...
                        prohibitedSubstances: [
                            ...
                            '{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}iurétiques',
                            'Bêta-bloquants',
                            'Cannabinoïdes'
                        ]
                    }
            ...
            mounted() {
                console.log('FIFA Ultimate {{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}ashboard optimized loaded successfully');
            ...
            methods: {
                initializeCharts() {
                    this.$nextTick(() => {
                        ...
                        this.createS{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}OHRadarChart();
                    });
                },
                createPerformanceChart() {
                    ...
                    const ratings = this.performanceEvolution.ratings || [75, 78, {{ $player->matchStats->sum("direction_changes") ?? 0 }}, 82, 85, 78];
                    ...
                    new Chart(ctx, {
                        ...
                        data: {
                            ...
                            datasets: [{
                                ...
                                tension: 0.4,
                                fill: true,
                                yAxisI{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}: 'y'
                            }, {
                                label: 'Buts marqués',
                                data: goals,
                                borderColor: '#10b981',
                                ...
                                tension: 0.4,
                                fill: false,
                                yAxisI{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}: 'y1'
                            }, {
                                label: 'Assists délivrés',
                                data: assists,
                                borderColor: '#f59e0b',
                                backgroundColor: 'rgba(2{{ $player->matchStats->count() }}, 158, 11, 0.1)',
                                tension: 0.4,
                                fill: false,
                                yAxisI{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}: 'y1'
                            }]
                    ...
                    new Chart(ctx, {
                        ...
                        data: {
                            ...
                            datasets: [{
                                ...
                                data: [{{ $player->matchStats->sum("direction_changes") ?? 0 }}, 78, 81, {{ $player->healthRecords->first() ? $player->healthRecords->first()->heart_rate : "N/A" }}, 79, 77],
                                borderColor: '#3b82f6',
                                backgroundColor: 'rgba(59, 130, 246, 0.1)',
                ...
                createS{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}OHRadarChart() {
                    ...
                    new Chart(ctx, {
                        ...
                        data: {
                            ...
                            datasets: [
                                {
                                    label: 'Score S{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}OH du joueur',
                                    data: [85, 85, 75, 80, 78],
                                    backgroundColor: 'rgba(20, 184, 166, 0.2)',
                                    borderColor: 'rgba(20, 184, 166, 1)',
                                    border{{ $player->matchStats->where("match_result", "win")->count() > 0 ? "W" : "L" }}idth: 2,
                                    pointBackgroundColor: 'rgba(20, 184, 166, 1)',
                                    pointBorderColor: '#fff',
                                    pointBorder{{ $player->matchStats->where("match_result", "win")->count() > 0 ? "W" : "L" }}idth: 2,
                                    pointHoverBackgroundColor: '#fff',
                                    pointHoverBorderColor: 'rgba(20, 184, 166, 1)',
                                    pointHoverBorder{{ $player->matchStats->where("match_result", "win")->count() > 0 ? "W" : "L" }}idth: 3,
                                    pointRadius: 7,
                                    pointHoverRadius: 10
                                },
                                {
                                    ...
                                    border{{ $player->matchStats->where("match_result", "win")->count() > 0 ? "W" : "L" }}idth: 1,
                                    pointBackgroundColor: 'rgba(99, 102, 241, 0.8)',
                                    pointBorderColor: '#fff',
                                    pointBorder{{ $player->matchStats->where("match_result", "win")->count() > 0 ? "W" : "L" }}idth: 1,
                                    pointRadius: 5,
                                    border{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}ash: [5, 5]
                                }
                            ]
                        },
                        options: {
                            ...
                            plugins: {
                                ...
                                title: {
                                    display: true,
                                    text: 'Facteurs sociaux déterminants de la santé (S{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}OH)',
                                    font: {
                                        size: 16,
                                        weight: 'bold'
                                    },
                                    color: '#{{ $player->date_of_birth ? \Carbon\Carbon::parse($player->date_of_birth)->age : "N/A" }}4151'
                                },
                                tooltip: {
                                    callbacks: {
        ...
        function shareSimple() {
            ...
            alert(`{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}ossier médical envoyé à ${email}`);
        ...
        function exportMedicalP{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}F() {
            alert('Export P{{ $player->matchStats->where("match_result", "draw")->count() > 0 ? "D" : "L" }}F en cours de développement');
        }
    </script>
</body>
</html>

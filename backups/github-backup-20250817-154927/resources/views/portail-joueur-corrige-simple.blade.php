        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            data() {
                return {
                    ...
                    offensiveStats: [
                        { name: 'Buts marqués', display: '{{ $player->performances->sum("goals") ?? 0 }}', percentage: 85, teamAvg: '8.5', leagueAvg: '7.2' },
                        ...
                        { name: 'Précision tirs', display: '{{ $player->performances->count() > 0 ? round(($player->performances->sum("shots_on_target") / $player->performances->sum("shots")) * 100) : 0 }}%', percentage: 78, teamAvg: '65%', leagueAvg: '62%' },
                        { name: 'Passes décisives', display: '{{ $player->performances->sum("assists") ?? 0 }}', percentage: 82, teamAvg: '6.8', leagueAvg: '5.9' },
                        { name: 'Passes clés', display: '{{ $player->performances->sum("key_passes") ?? 0 }}', percentage: 78, teamAvg: '22.1', leagueAvg: '19.8' },
                        { name: 'Centres réussis', display: '{{ $player->performances->sum("crosses") ?? 0 }}', percentage: 75, teamAvg: '15.3', leagueAvg: '13.7' },
                        { name: 'Dribbles réussis', display: '{{ $player->performances->sum("dribbles") ?? 0 }}', percentage: 82, teamAvg: '{{ $player->performances->count() }}.2', leagueAvg: '38.9' }
                    ],
                    physicalStats: [
                        { name: 'Distance parcourue', display: '{{ $player->performances->sum("distance_covered") ?? 0 }} km', percentage: 78, teamAvg: '198 km', leagueAvg: '185 km' },
                        { name: 'Vitesse maximale', display: '{{ $player->performances->max("max_speed") ?? 0 }} km/h', percentage: 85, teamAvg: '32.1 km/h', leagueAvg: '30.8 km/h' },
                        { name: 'Vitesse moyenne', display: '{{ $player->performances->avg("avg_speed") ?? 0 }} km/h', percentage: 82, teamAvg: '11.2 km/h', leagueAvg: '10.9 km/h' },
                        { name: 'Sprints', display: '{{ $player->performances->sum("sprints") ?? 0 }}', percentage: 82, teamAvg: '{{ $player->performances->count() > 0 ? round($player->performances->avg("sprints")) : 0 }}', leagueAvg: '115' },

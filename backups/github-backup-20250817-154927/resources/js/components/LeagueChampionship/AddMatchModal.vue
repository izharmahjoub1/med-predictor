  ...
  <div class="modal-overlay" @click="$emit('close')">
    <div class="modal-content" @click.stop>
      ...
      <form @submit.prevent="submitForm" class="space-y-4">
        <!-- Matchday -->
        <div>
          <label class="block text-sm font-medium mb-2">{{ $t('auto.key526') }}</label>
          <input
            {{ $t('auto.key3') }}
            {{ $t('auto.key4') }}
            {{ $t('auto.key5') }}
        ...
        <!-- Home Team -->
        <div>
          <label class="block text-sm font-medium mb-2">{{ $t('auto.key527') }}</label>
          <select
            {{ $t('auto.key8') }}
            {{ $t('auto.key6') }}
            {{ $t('auto.key7') }}
          >
            <option value="">{{ $t('auto.key528') }}</option>
            <option
              {{ $t('auto.key11') }}
              :key="team.id"
              :value="team.id"
        ...
        <!-- Away Team -->
        <div>
          <label class="block text-sm font-medium mb-2">{{ $t('auto.key529') }}</label>
          <select
            {{ $t('auto.key13') }}
            {{ $t('auto.key6') }}
            {{ $t('auto.key7') }}
          >
            <option value="">{{ $t('auto.key530') }}</option>
            <option
              {{ $t('auto.key16') }}
              :key="team.id"
              :value="team.id"
        ...
        <!-- Date and Time -->
        <div>
          <label class="block text-sm font-medium mb-2">{{ $t('auto.key531') }}</label>
          <input
            {{ $t('auto.key18') }}
            {{ $t('auto.key19') }}
            {{ $t('auto.key6') }}
        ...
        <!-- Venue -->
        <div>
          <label class="block text-sm font-medium mb-2">{{ $t('auto.key532') }}</label>
          <select
            {{ $t('auto.key22') }}
            {{ $t('auto.key7') }}
          >
            <option value="">{{ $t('auto.key533') }}</option>
            <option
              {{ $t('auto.key24') }}
              :key="venue"
              :value="venue"
        ...
        <div class="flex gap-4 pt-4">
          ...
          >
            <span v-if="isSubmitting">{{ $t('auto.key534') }}</span>

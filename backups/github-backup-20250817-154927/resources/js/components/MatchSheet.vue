  ...
  <div class="container mx-auto py-8">
    <div class="bg-white rounded-lg shadow-lg p-6">
      <div class="flex justify-between items-center mb-6">
        <h2 class="text-xl font-bold">{{ $t('auto.key124') }}</h2>
        <button @click="goBack" class="bg-gray-500 text-white px-4 py-2 rounded hover:bg-gray-600">
          Retour au dashboard
        </button>
      </div>
      <div v-if="loading" class="text-gray-500">{{ $t('auto.key125') }}</div>
      <div v-else>
        <div class="mb-4">
          <div class="flex justify-between">
            <div>
              <span class="font-semibold">{{ $t('auto.key126') }}</span> {{ match?.competition?.name }}<br>
              <span class="font-semibold">{{ $t('auto.key127') }}</span> {{ formatDate(match?.match_date) }}
            </div>
            <div>
              <span class="font-semibold">{{ $t('auto.key128') }}</span> <span :class="statusColor(matchSheet?.status)">{{ statusLabel(matchSheet?.status) }}</span>
            </div>
          </div>
          <div class="mt-2">
            <span class="font-semibold">{{ $t('auto.key129') }}</span> {{ match?.home_team?.name }}
            <span class="mx-2 font-bold">{{ matchSheet?.home_team_score }} - {{ matchSheet?.away_team_score }}</span>
            <span class="font-semibold">{{ $t('auto.key130') }}</span> {{ match?.away_team?.name }}
          </div>
        </div>
        <div class="mb-4 flex gap-4">
          <div>
            <label class="block text-sm font-medium">{{ $t('auto.key131') }}</label>
            <input type="number" v-model.number="form.home_team_score" min="0" class="border rounded px-2 py-1 w-20" />
          </div>
          <div>
            <label class="block text-sm font-medium">{{ $t('auto.key132') }}</label>
            <input type="number" v-model.number="form.away_team_score" min="0" class="border rounded px-2 py-1 w-20" />
          </div>
        </div>
        <div class="mb-4">
          <div class="flex items-center justify-between mb-2">
            <span class="font-semibold">{{ $t('auto.key133') }}</span>
            <button class="bg-green-600 text-white px-3 py-1 rounded hover:bg-green-700" @click="showEventModal = true">{{ $t('auto.key134') }}</button>
          </div>
          <ul>
            <li v-for="(event, idx) in form.events" :key="idx" class="mb-1 flex items-center gap-2">
              ...
              <button class="ml-2 text-red-600 hover:underline" @click="removeEvent(idx)">{{ $t('auto.key135') }}</button>
            </li>
          </ul>
        </div>
        <div class="mb-4">
          <label class="block text-sm font-medium">{{ $t('auto.key136') }}</label>
          <textarea v-model="form.notes" class="border rounded px-2 py-1 w-full" rows="2"></textarea>
        </div>
        <div class="flex gap-4 justify-end">
          <button class="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700" @click="saveDraft" :disabled="saving">{{ $t('auto.key137') }}</button>
          <button class="bg-indigo-600 text-white px-4 py-2 rounded hover:bg-indigo-700" @click="submitSheet" :disabled="submitting">{{ $t('auto.key138') }}</button>
        </div>

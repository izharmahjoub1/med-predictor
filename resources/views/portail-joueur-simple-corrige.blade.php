...
<head>
    ...
    <style>
        ...
        .fifa-ultimate-card {
            background: linear-gradient(135deg, #1a2a3e 0%, #303f9f 25%, #3f51b5 50%, #5c6bc0 75%, #9c27b0 100%);
            position: relative;
            overflow: hidden;
            border-radius: 24px;
                            ...
                            <div class="relative w-20 h-20 mb-2">
                                <svg class="w-full h-full progress-circle" viewBox="0 0 100 100">
                                    ...
                                    <circle cx="50" cy="50" r="40" stroke="#10b981" stroke-width="8" fill="none"
                                            stroke-dasharray="251.33" stroke-dashoffset="{{ $player->date_of_birth ? \Carbon\Carbon::parse($player->date_of_birth)->age : "N/A" }}" stroke-linecap="round"/>
                                </svg>
            ...
            data() {
                return {
                    ...
                    physicalStats: [
                        ...
                        { name: 'Décélérations', display: '{{ $player->performances->sum("decelerations") ?? 0 }}', percentage: {{ $player->matchStats->sum("goals_scored") > 0 ? 85 : 70 }}, teamAvg: '172', leagueAvg: '165' },
                    ...
                    technicalStats: [
                        ...
                        { name: 'Centres réussis', display: '{{ $player->matchStats->sum("crosses_total") ?? 0 }}', percentage: {{ $player->matchStats->sum("crosses_total") > 0 ? 75 : 65 }}, teamAvg: '{{ $player->matchStats->count() > 0 ? $player->matchStats->count() > 0 ? round($player->performances->avg("crosses"), 1) : 0 : 0 }}', leagueAvg: '13.7' },
                        { name: 'Tacles réussis', display: '{{ $player->performances->sum("tackles") ?? 0 }}', percentage: {{ $player->matchStats->sum("assists_provided") > 0 ? 82 : 70 }}, teamAvg: '12.5', leagueAvg: '26.8' },
                    ...
                    seasonSummary: {
                        ...
                        'matches': {'total': {{ $player->matchStats->sum("goals_scored") ?? 0 }}, 'rating': {{ $player->matchStats->count() > 0 ? round($player->matchStats->avg("match_rating"), 1) : 0 }}, 'distance': '28 km'}
                    },
                    ...
                    notificationData: {
                        ...
                        'socialAlerts': [
                            ...
                            { id: 3, title: 'Instagram Live', content: 'Solicitud de entrevista en vivo', timestamp: '04/03/2025', views: '8.5K', engagement: '83', sentiment: 'neutral', platform: 'instagram', needsResponse: true }
                        ]
                    },
                    healthData: {
                        'globalScore': 87,
                        ...
                        recentMetrics: [
                            { id: 1, type: 'Poids Corporel', value: '75.1 kg', date: '10/03/2025', icon: 'fas fa-weight', color: '#3b82f6', trend: 'stable', change: '+0.1kg' },
                    ...
                    dopingData: {
                        lastControl: '12/02/2025',
                        nextControl: '15/04/2025',
                        controlHistory: [
                            { id: 1, type: 'Contrôle inopiné', date: '12/02/2025', location: 'Centre d\'entraînement', status: 'completed', result: 'négatif', substances: 'Aucune substance interdite détectée', notes: 'Contrôle standard, aucun problème détecté' },

            ...
            <div class="p-6 text-gray-900">
                <div class="flex justify-between items-center mb-6">
                    ...
                    <div class="flex space-x-3">
                        <a href="{{ route('club-management.teams.show', $team->id) }}" class="inline-flex items-center px-4 py-2 bg-gray-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700 focus:bg-gray-700 active:bg-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150">
                            Back to Team
                        </a>
                ...
                <form method="POST" action="{{ route('club-management.teams.update', $team->id) }}" class="space-y-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <select name="type" id="type" required
                                ...
                                @foreach($teamTypes as $value => $label)
                                    <option value="{{ $value }}" {{ old('type', $team->type) == $value ? 'selected' : '' }}>
                                        {{ $label }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <select name="formation" id="formation" required
                                ...
                                @foreach($formations as $value => $label)
                                    <option value="{{ $value }}" {{ old('formation', $team->formation) == $value ? 'selected' : '' }}>
                                        {{ $label }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <input type="text" name="season" id="season" value="{{ old('season', $team->season) }}" required
                                class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                            @error('season')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <input type="text" name="coach_name" id="coach_name" value="{{ old('coach_name', $team->coach_name) }}"
                                class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                            @error('coach_name')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <input type="number" name="budget_allocation" id="budget_allocation" value="{{ old('budget_allocation', $team->budget_allocation) }}" step="0.01" min="0"
                                class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                    ...
                    <div>
                        ...
                        <input type="text" name="tactical_style" id="tactical_style" value="{{ old('tactical_style', $team->tactical_style) }}"
                            class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                    ...
                    <div>
                        ...
                        <textarea name="playing_philosophy" id="playing_philosophy" rows="3"
                            class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">{{ old('playing_philosophy', $team->playing_philosophy) }}</textarea>
                    ...
                    <div class="flex justify-end space-x-3">
                        <a href="{{ route('club-management.teams.show', $team->id) }}" 

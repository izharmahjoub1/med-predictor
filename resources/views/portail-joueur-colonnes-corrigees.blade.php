        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            data() {
                return {
                    ...
                    offensiveStats: [
                        { name: 'Buts marqués', display: '{{ $player->performances->sum("goals") ?? 0 }}', percentage: {{ $player->performances->sum("goals") > 0 ? 85 : 70 }}, teamAvg: '{{ $player->performances->count() > 0 ? $player->performances->count() > 0 ? round($player->performances->avg("goals"), 1) : 0 : 0 }}', leagueAvg: '7.2' },
                        { name: 'Tirs cadrés', display: '{{ $player->performances->sum("shots_on_target") ?? 0 }}', percentage: {{ $player->performances->sum("shots") > 0 ? 78 : 65 }}, teamAvg: '{{ $player->performances->count() > 0 ? round($player->performances->avg("shots_on_target"), 1) : 0 }}', leagueAvg: '{{ $player->performances->count() > 0 ? round($player->performances->avg("shots_on_target") * 1.1, 1) : 0 }}' },
                        { name: 'Tirs totaux', display: '{{ $player->performances->sum("shots") ?? 0 }}', percentage: 72, teamAvg: '{{ $player->performances->count() > 0 ? round($player->performances->avg("shots"), 1) : 0 }}', leagueAvg: '{{ $player->performances->count() > 0 ? round($player->performances->avg("shots") * 1.1, 1) : 0 }}' },
                        { name: 'Précision tirs', display: '{{ $player->performances->sum("shots") > 0 ? round(($player->performances->sum("shots_on_target") / $player->performances->sum("shots")) * 100) : 0 }}%', percentage: {{ $player->performances->sum("shots") > 0 ? 78 : 65 }}, teamAvg: '{{ $player->performances->sum("shots") > 0 ? round(($player->performances->sum("shots_on_target") / $player->performances->sum("shots")) * 100) : 0 }}%', leagueAvg: '{{ $player->performances->sum("shots") > 0 ? round(($player->performances->sum("shots_on_target") / $player->performances->sum("shots")) * 100 * 0.95) : 0 }}%' },
                        { name: 'Passes décisives', display: '{{ $player->performances->sum("assists") ?? 0 }}', percentage: {{ $player->performances->sum("assists") > 0 ? 82 : 70 }}, teamAvg: '{{ $player->performances->count() > 0 ? $player->performances->count() > 0 ? round($player->performances->avg("assists"), 1) : 0 : 0 }}', leagueAvg: '5.9' },
                        { name: 'Passes clés', display: '{{ $player->performances->sum("key_passes") ?? 0 }}', percentage: {{ $player->performances->sum("shots") > 0 ? 78 : 65 }}, teamAvg: '{{ $player->performances->count() > 0 ? round($player->performances->avg("key_passes"), 1) : 0 }}', leagueAvg: '{{ $player->performances->count() > 0 ? round($player->performances->avg("key_passes") * 1.1, 1) : 0 }}' },
                        { name: 'Centres réussis', display: '{{ $player->performances->sum("crosses") ?? 0 }}', percentage: {{ $player->performances->sum("crosses") > 0 ? 75 : 65 }}, teamAvg: '{{ $player->performances->count() > 0 ? $player->performances->count() > 0 ? round($player->performances->avg("crosses"), 1) : 0 : 0 }}', leagueAvg: '13.7' },
                        { name: 'Dribbles réussis', display: '{{ $player->performances->sum("dribbles") ?? 0 }}', percentage: {{ $player->performances->sum("assists") > 0 ? 82 : 70 }}, teamAvg: '{{ $player->performances->count() > 0 ? round($player->performances->avg("dribbles"), 1) : 0 }}', leagueAvg: '38.9' }
                    ],
                    physicalStats: [
                        { name: 'Distance parcourue', display: '{{ $player->performances->sum("distance_covered") ?? 0 }} km', percentage: {{ $player->performances->sum("shots") > 0 ? 78 : 65 }}, teamAvg: '{{ $player->performances->count() > 0 ? round($player->performances->avg("distance_covered")) : 0 }} km', leagueAvg: '{{ $player->performances->count() > 0 ? round($player->performances->avg("distance_covered") * 0.95) : 0 }} km' },
                        { name: 'Vitesse maximale', display: '{{ $player->performances->max("max_speed") ?? 0 }} km/h', percentage: {{ $player->performances->sum("goals") > 0 ? 85 : 70 }}, teamAvg: '{{ $player->performances->count() > 0 ? round($player->performances->avg("max_speed"), 1) : 0 }} km/h', leagueAvg: '{{ $player->performances->count() > 0 ? round($player->performances->avg("max_speed") * 0.95, 1) : 0 }} km/h' },
                        { name: 'Vitesse moyenne', display: '{{ $player->performances->avg("avg_speed") ?? 0 }} km/h', percentage: {{ $player->performances->sum("assists") > 0 ? 82 : 70 }}, teamAvg: '{{ $player->performances->count() > 0 ? round($player->performances->avg("avg_speed"), 1) : 0 }} km/h', leagueAvg: '{{ $player->performances->count() > 0 ? round($player->performances->avg("avg_speed") * 0.95, 1) : 0 }} km/h' },
                        { name: 'Sprints', display: '{{ $player->performances->sum("sprints") ?? 0 }}', percentage: {{ $player->performances->sum("assists") > 0 ? 82 : 70 }}, teamAvg: '{{ $player->performances->count() > 0 ? $player->performances->count() > 0 ? round($player->performances->avg("sprints")) : 0 : 0 }}', leagueAvg: '115' },
                        { name: 'Accélérations', display: '{{ $player->performances->sum("accelerations") ?? 0 }}', percentage: {{ $player->performances->sum("accelerations") > 0 ? 87 : 75 }}, teamAvg: '{{ $player->performances->count() > 0 ? round($player->performances->avg("accelerations")) : 0 }}', leagueAvg: '{{ $player->performances->count() > 0 ? round($player->performances->avg("accelerations") * 0.95) : 0 }}' },
                        { name: 'Décélérations', display: '{{ $player->performances->sum("decelerations") ?? 0 }}', percentage: {{ $player->performances->sum("goals") > 0 ? 85 : 70 }}, teamAvg: '1{{ $player->healthRecords->first() ? $player->healthRecords->first()->heart_rate : "N/A" }}', leagueAvg: '165' },
                        { name: 'Changements direction', display: '{{ $player->performances->sum("direction_changes") ?? 0 }}', percentage: {{ $player->performances->sum("shots") > 0 ? 78 : 65 }}, teamAvg: '{{ $player->performances->sum("direction_changes") ?? 0 }}', leagueAvg: '{{ $player->healthRecords->first() ? $player->healthRecords->first()->heart_rate : "N/A" }}' },
                        { name: 'Sautes', display: '{{ $player->performances->sum("goals") ?? 0 }}', percentage: 70, teamAvg: '15.2', leagueAvg: '14.8' }
                    ],
                    technicalStats: [
                        { name: 'Précision passes', display: '{{ $player->performances->sum("passes") > 0 ? round(($player->performances->sum("passes_completed") / $player->performances->sum("passes")) * 100) : 0 }}%', percentage: {{ $player->performances->sum("assists") > 0 ? 82 : 70 }}, teamAvg: '{{ $player->performances->sum("passes") > 0 ? round(($player->performances->sum("passes_completed") / $player->performances->sum("passes")) * 100) : 0 }}%', leagueAvg: '{{ $player->performances->sum("passes") > 0 ? round(($player->performances->sum("passes_completed") / $player->performances->sum("passes")) * 100 * 0.95) : 0 }}%' },
                        { name: 'Passes longues', display: '{{ $player->performances->count() }}', percentage: {{ $player->performances->sum("shots") > 0 ? 78 : 65 }}, teamAvg: '38', leagueAvg: '35' },
                        { name: 'Centres réussis', display: '{{ $player->performances->sum("crosses") ?? 0 }}', percentage: {{ $player->performances->sum("crosses") > 0 ? 75 : 65 }}, teamAvg: '{{ $player->performances->count() > 0 ? $player->performances->count() > 0 ? round($player->performances->avg("crosses"), 1) : 0 : 0 }}', leagueAvg: '13.7' },
                        { name: 'Tacles réussis', display: '{{ $player->performances->sum("tackles") ?? 0 }}', percentage: {{ $player->performances->sum("assists") > 0 ? 82 : 70 }}, teamAvg: '{{ $player->performances->sum("goals") ?? 0 }}.5', leagueAvg: '26.8' },
                        { name: 'Interceptions', display: '{{ $player->performances->sum("interceptions") ?? 0 }}', percentage: {{ $player->performances->sum("goals") > 0 ? 85 : 70 }}, teamAvg: '26.8', leagueAvg: '24.5' },
                        { name: 'Dégagements', display: '{{ $player->performances->sum("goals") ?? 0 }}', percentage: {{ $player->performances->sum("shots") > 0 ? 78 : 65 }}, teamAvg: '15.2', leagueAvg: '14.1' },
                        { name: 'Fautes commises', display: '{{ $player->performances->sum("assists") ?? 0 }}', percentage: {{ $player->performances->sum("assists") > 0 ? 82 : 70 }}, teamAvg: '12.4', leagueAvg: '11.8' },
                        { name: 'Cartons jaunes', display: '{{ $player->performances->sum("yellow_cards") ?? 0 }}', percentage: {{ $player->performances->sum("goals") > 0 ? 85 : 70 }}, teamAvg: '{{ $player->performances->count() > 0 ? round($player->performances->avg("yellow_cards"), 1) : 0 }}', leagueAvg: '{{ $player->performances->count() > 0 ? round($player->performances->avg("yellow_cards") * 1.1, 1) : 0 }}' }
                    ],
                    seasonSummary: {

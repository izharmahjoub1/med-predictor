...
<html lang="fr">
<head>
    <meta charset="UTF-{{ $player->pcmas->count() }}">
    ...
    @php
        // Variables pour éviter les conflits de syntaxe dans le JavaScript
        $countries1 = ['Brasil', 'Uruguay', 'Argentine', 'Allemagne', 'Espagne'];
        $countries2 = ['Portugal', 'Pays-Bas', 'Italie', 'Angleterre', 'Belgique'];
        $clubs = ['Manchester City', 'Arsenal', 'Liverpool', 'Tottenham', 'Manchester United'];
        $competitions = ['Premier League', 'Champions League', 'FA Cup', 'Carabao Cup'];
        $times = ['00', '15', '30', '45'];
        $selectedCountry1 = $countries1[rand(0, 4)];
        $selectedCountry2 = $countries2[rand(0, 4)];
        $selectedClub = $clubs[rand(0, 4)];
        $selectedCompetition = $competitions[rand(0, 3)];
        $selectedTime1 = $times[rand(0, 3)];
        $selectedTime2 = $times[rand(0, 3)];
        $selectedTime3 = $times[rand(0, 3)];
        $selectedTime4 = $times[rand(0, 3)];
        
        // Variables pour les valeurs par défaut des charts
        $defaultRatings = [7.2, 7.4, 7.6, 7.7, 7.8, 7.7];
        $defaultGoals = [2, 3, 2, 1, 2, 2];
        $defaultAssists = [1, 2, 1, 1, 2, 1];
        
        // Valeur par défaut pour fifa_average
        $defaultFifaAverage = [
            'environment' => 75,
            'social_support' => 70,
            'healthcare_access' => 80,
            'financial_situation' => 75,
            'mental_wellbeing' => 70
        ];
    @endphp
    ...
    <style>
        ...
        .fifa-ultimate-card:hover {
            transform: translateY(-{{ $player->pcmas->count() }}px) scale(1.02);
            box-shadow: 0 35px 70px rgba(0,0,0,0.5), inset 0 1px 0 rgba(255,255,255,0.3);
        }

        .fifa-rating-badge {
            background: linear-gradient(135deg, #ffd700 0%, #ffb300 50%, #ff{{ $player->pcmas->count() }}f00 100%);
            border-radius: 16px;
            box-shadow: 0 {{ $player->pcmas->count() }}px 20px rgba(255, 215, 0, 0.4), inset 0 2px 4px rgba(255,255,255,0.3);
            border: 3px solid #fff;
        }

        .fifa-nav-tab {
            background: linear-gradient(135deg, #1e3a{{ $player->pcmas->count() }}a, #1e40af);
            backdrop-filter: blur(10px);
            border: 1px solid rgba(59, 130, 246, 0.3);
            transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1);
            box-shadow: 0 4px {{ $player->healthRecords->count() }}px rgba(30, 5{{ $player->pcmas->count() }}, 13{{ $player->pcmas->count() }}, 0.3);
        }
        ...
        .fifa-nav-tab.active {
            ...
            box-shadow: 0 {{ $player->pcmas->count() }}px 20px rgba(30, 5{{ $player->pcmas->count() }}, 13{{ $player->pcmas->count() }}, 0.4);
        }

        .fifa-nav-tab:hover {
            background: linear-gradient(135deg, #1e40af, #3b{{ $player->pcmas->count() }}2f6);
            transform: translateY(-1px);
            box-shadow: 0 6px 16px rgba(30, 5{{ $player->pcmas->count() }}, 13{{ $player->pcmas->count() }}, 0.35);
        }

        .fifa-position-badge {
            background: linear-gradient(135deg, #dc2626 0%, #ef4444 50%, #f{{ $player->pcmas->count() }}7171 100%);
            border-radius: {{ $player->pcmas->count() }}px;
            box-shadow: 0 4px {{ $player->healthRecords->count() }}px rgba(220, 3{{ $player->pcmas->count() }}, 3{{ $player->pcmas->count() }}, 0.4);
            border: 2px solid #fff;
        }

        .stat-bar {
            height: {{ $player->pcmas->count() }}px;
            background: linear-gradient({{ $player->ghs_civic_score ?? "N/A" }}deg, #10b9{{ $player->pcmas->count() }}1 0%, #34d399 100%);
            border-radius: 4px;
            transition: width 1s ease-out;
        }

        .health-indicator {
            width: {{ $player->healthRecords->count() }}px;
            height: {{ $player->healthRecords->count() }}px;
            border-radius: 50%;
            margin-right: {{ $player->pcmas->count() }}px;
        }
        
        .health-excellent { background: #10b9{{ $player->pcmas->count() }}1; }
        .health-good { background: #f59e0b; }
        .health-warning { background: #ef4444; }

        .notification-badge {
            position: absolute;
            top: -{{ $player->pcmas->count() }}px;
            right: -{{ $player->pcmas->count() }}px;
            background: linear-gradient(135deg, #dc2626, #ef4444);
            color: white;
            border: 2px solid rgba(255,255,255,0.3);
            box-shadow: 0 2px {{ $player->pcmas->count() }}px rgba(220, 3{{ $player->pcmas->count() }}, 3{{ $player->pcmas->count() }}, 0.4);
            border-radius: 50%;
            width: 20px;
            height: 20px;
            font-size: {{ $player->healthRecords->count() }}px;
            display: flex;
            align-items: center;
        ...
        .performance-card {
            background: rgba(255, 255, 255, 0.{{ $player->ghs_mental_score ?? "N/A" }});
            backdrop-filter: blur(10px);
            border-radius: 16px;
            box-shadow: 0 {{ $player->pcmas->count() }}px 32px rgba(0, 0, 0, 0.1);
            transition: all 0.3s ease;
        }

        .performance-card:hover {
            transform: translateY(-4px);
            box-shadow: 0 {{ $player->healthRecords->count() }}px 4{{ $player->pcmas->count() }}px rgba(0, 0, 0, 0.15);
        }
        ...
        .progress-circle {
            transform: rotate(-{{ $player->ghs_civic_score ?? "N/A" }}deg);
        }
    ...
    <div id="fifa-app">
        ...
        <div class="fifa-ultimate-card text-white p-6 m-4 relative">
            ...
            <div class="absolute inset-0 overflow-hidden">
                ...
                <div class="absolute w-4{{ $player->pcmas->count() }} h-4{{ $player->pcmas->count() }} bg-white/3 rounded-full -bottom-24 -left-24"></div>
            </div>
                ...
                <div class="flex flex-col lg:flex-row items-center justify-between space-y-6 lg:space-y-0">
                    ...
                    <div class="flex items-center space-x-6">
                        ...
                        <div class="relative flex-shrink-0">
                            <div class="w-32 h-32 lg:w-{{ $player->date_of_birth ? $player->date_of_birth->diffInYears(now()) : "N/A" }} lg:h-{{ $player->date_of_birth ? $player->date_of_birth->diffInYears(now()) : "N/A" }} rounded-full overflow-hidden border-4 border-white/20 shadow-2xl bg-gradient-to-br from-gray-400 to-gray-600">
                                <!-- Image avec multiples fallbacks -->
                                <img :src="images.player_profile" 
                                     alt="{{ $player->first_name }} {{ $player->last_name }}"
                                     class="w-full h-full object-cover"
                                     onerror="this.onerror=null; this.src='https://via.placeholder.com/200x200/cccccc/666666?text={{ $player->first_name }}+{{ $player->last_name }}'; if(this.complete && this.naturalHeight == 0) { this.style.display='none'; this.nextElementSibling.style.display='flex'; }">
                        ...
                        <div class="text-center lg:text-left">
                            ...
                            <div class="flex items-center justify-center lg:justify-start space-x-4 mb-4">
                                ...
                                <div class="flex items-center space-x-2">
                                    <img :src="images.club_logo" 
                                         alt="{{ $player->club?->name ?? 'Club' }}" 
                                         class="w-8 h-8 object-contain"
                                         onerror="this.onerror=null; this.src='https://via.placeholder.com/32x32/cccccc/666666?text={{ $player->club?->name ?? 'C' }}'; if(this.complete && this.naturalHeight == 0) { this.style.display='none'; this.nextElementSibling.style.display='inline-block'; }">
                                    <!-- Fallback club icon -->
                                    <div class="w-{{ $player->pcmas->count() }} h-{{ $player->pcmas->count() }} bg-blue-500 rounded-full flex items-center justify-center text-white text-xs font-bold" style="display: none;">
                                        C
                                    </div>
                                ...
                                <div class="flex items-center space-x-2">
                                    <img :src="images.player_flag" 
                                         alt="{{ $player->nationality ?? 'Pays' }}" 
                                         class="w-6 h-4 object-cover rounded"
                                         onerror="this.onerror=null; this.src='https://via.placeholder.com/24x16/cccccc/666666?text={{ $player->nationality ?? 'P' }}'; if(this.complete && this.naturalHeight == 0) { this.style.display='none'; this.nextElementSibling.style.display='inline-block'; }">
                                    <!-- Fallback nation -->
                            ...
                            <div class="grid grid-cols-2 md:grid-cols-2 gap-3 text-sm mb-4">
                                <div class="text-center p-2 bg-white/10 rounded-lg">
                                    <div class="font-semibold text-yellow-400">@{{ playerStats.age || 'N/A' }}</div>
                                    <div class="text-xs opacity-70">Âge</div>
                                </div>
                                <div class="text-center p-2 bg-white/10 rounded-lg">
                                    <div class="font-semibold text-blue-400">@{{ playerStats.height ? playerStats.height + 'cm' : 'N/A' }}</div>
                                    <div class="text-xs opacity-70">Taille</div>
                                </div>
                                <div class="text-center p-2 bg-white/10 rounded-lg">
                                    <div class="font-semibold text-green-400">@{{ playerStats.weight ? playerStats.weight + 'kg' : 'N/A' }}</div>
                                    <div class="text-xs opacity-70">Poids</div>
                                </div>
                                <div class="text-center p-2 bg-white/10 rounded-lg">
                                    <div class="font-semibold text-purple-400">@{{ playerStats.preferred_foot || 'N/A' }}</div>
                            ...
                            <div class="flex flex-wrap justify-center lg:justify-start gap-2 mb-3">
                                <span class="px-2 py-1 bg-yellow-500 text-black text-xs font-bold rounded-full">🏆 @{{ playerStats.ballon_dor_count }}x Ballon d'Or</span>
                                <span class="px-2 py-1 bg-blue-500 text-white text-xs font-bold rounded-full">⚽ @{{ playerStats.total_goals }}+ Buts</span>
                                <span class="px-2 py-1 bg-green-500 text-white text-xs font-bold rounded-full">🎯 @{{ playerStats.total_assists }}+ Assists</span>
                                <span class="px-2 py-1 bg-purple-500 text-white text-xs font-bold rounded-full">🏆 @{{ playerStats.champions_league_count }}x Champions League</span>
                            </div>

                            <!-- Statistiques saison actuelle -->
                            <div class="grid grid-cols-3 gap-2 text-center text-xs">
                                <div class="p-2 bg-white/5 rounded">
                                    <div class="font-bold text-green-400">@{{ playerStats.season_goals }}</div>
                                    <div class="opacity-70">Buts saison</div>
                                </div>
                                <div class="p-2 bg-white/5 rounded">
                                    <div class="font-bold text-blue-400">@{{ playerStats.season_assists }}</div>
                                    <div class="opacity-70">Assists</div>
                                </div>
                                <div class="p-2 bg-white/5 rounded">
                                    <div class="font-bold text-purple-400">@{{ playerStats.total_goals }}</div>
                                    <div class="opacity-70">Total buts</div>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="flex flex-col lg:flex-row items-center space-y-4 lg:space-y-0 lg:space-x-6">
                        ...
                        <div class="text-center">
                            <div class="flex space-x-3 mb-2">
                                <div class="fifa-rating-badge text-black p-4">
                                    <div class="text-3xl font-bold" v-text="fifaStats.overall_rating"></div>
                                    <div class="text-xs">OVR</div>
                                </div>
                                <div class="fifa-rating-badge text-black p-3">
                                    <div class="text-2xl font-bold" v-text="fifaStats.potential_rating"></div>
                                    <div class="text-xs">POT</div>
                                </div>
                            </div>
                            <div class="text-xs text-white/{{ $player->ghs_civic_score ?? "N/A" }}">FIFA Ultimate</div>
                        </div>
                        ...
                        <div class="text-center">
                            <div class="relative w-20 h-20 mb-2">
                                <svg class="w-full h-full progress-circle" viewBox="0 0 100 100">
                                    <circle cx="50" cy="50" r="40" stroke="rgba(255,255,255,0.2)" stroke-width="{{ $player->pcmas->count() }}" fill="none"/>
                                    <circle cx="50" cy="50" r="40" stroke="#10b9{{ $player->pcmas->count() }}1" stroke-width="{{ $player->pcmas->count() }}" fill="none"
                                            stroke-dasharray="251.33" stroke-dashoffset="37.7" stroke-linecap="round"/>
                                </svg>
                                <div class="absolute inset-0 flex items-center justify-center">
                                    <span class="text-xl font-bold text-green-400" v-text="fifaStats.fitness_score"></span>
                                </div>
                            </div>
                        ...
                        <div class="space-y-2 text-sm">
                            <div class="p-3 bg-gradient-to-r from-green-500/20 to-green-600/20 rounded-lg border border-green-500/30">
                                ...
                                <div class="font-bold text-green-400" v-text="fifaStats.injury_risk + '% - ' + (fifaStats.injury_risk <= 15 ? 'FAIBLE' : fifaStats.injury_risk <= 25 ? 'MOYEN' : 'ÉLEVÉ')"></div>
                                <div class="w-full bg-green-500/50 rounded-full h-1 mt-1">
                                    <div class="bg-green-500 h-1 rounded-full" :style="'width: ' + fifaStats.injury_risk + '%'"></div>
                                </div>
                            </div>
                            ...
                            <div class="p-3 bg-gradient-to-r from-yellow-500/20 to-yellow-600/20 rounded-lg border border-yellow-500/30">
                                ...
                                <div class="font-bold text-yellow-400" v-text="'€' + fifaStats.market_value + 'M'"></div>
                            ...
                            <div class="p-3 bg-gradient-to-r from-blue-500/20 to-blue-600/20 rounded-lg border border-blue-500/30">
                                ...
                                <div class="font-bold text-blue-400" v-text="fifaStats.availability === 'available' ? '✅ DISPONIBLE' : '❌ INDISPONIBLE'"></div>
                            ...
                            <div class="grid grid-cols-2 gap-1 text-xs">
                                <div class="text-center p-2 bg-white/5 rounded">
                                    <div class="font-bold text-orange-400" v-text="fifaStats.form_percentage + '%'"></div>
                                    <div class="opacity-70">Forme</div>
                                </div>
                                <div class="text-center p-2 bg-white/5 rounded">
                                    <div class="font-bold text-cyan-400" v-text="fifaStats.morale_percentage + '%'"></div>
                ...
                <div class="mt-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div class="lg:col-span-2 p-4 bg-white/5 rounded-xl backdrop-blur-sm">
                        <div class="flex justify-between items-center mb-2">
                            <span class="text-sm font-medium">Saison @{{ seasonProgress.currentSeason }}</span>
                            <span class="text-sm text-white/{{ $player->pcmas->count() }}0">@{{ seasonProgress.completion }}% complétée</span>
                        </div>
                        <div class="w-full bg-white/20 rounded-full h-2 mb-2">
                            <div class="bg-gradient-to-r from-blue-500 to-purple-600 h-2 rounded-full" :style="'width: ' + seasonProgress.completion + '%'"></div>
                        </div>
                        <div class="flex justify-between text-xs text-white/70">
                            <span>@{{ seasonProgress.matchesPlayed }} matchs joués</span>
                            <span>@{{ seasonProgress.matchesRemaining }} matchs restants</span>
                        </div>
                    </div>
                    ...
                    <div class="p-4 bg-white/5 rounded-xl backdrop-blur-sm">
                        ...
                        <div class="flex justify-between items-center text-xs">
                            ...
                            <div v-for="(match, index) in recentPerformances" :key="index"
                                 :class="['w-4 h-4 rounded-full flex items-center justify-center text-white text-xs cursor-pointer relative group', 
                                          match.result === 'W' ? 'bg-green-500' : (match.result === 'D' ? 'bg-yellow-500' : 'bg-red-500')]"
                                 :title="match.date + ' vs ' + (match.opponent || 'Adversaire')">
                                @{{ match.result }}
                                <div class="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-1 px-2 py-1 bg-gray-800 text-white text-xs rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap z-10">
                                    @{{ match.date }} vs @{{ match.opponent || 'Adversaire' }}
                                    <br>@{{ match.competition || 'Compétition' }}
                                    <br>Buts: @{{ match.goals || 0 }} | Passes: @{{ match.assists || 0 }}
                                </div>
                            </div>
                        </div>
                        </div>
        ...
        <div class="px-6 pb-6">
            ...
            <div v-show="activeTab === 'performance'" class="space-y-6">
                ...
                <div class="performance-card p-6">
                    <h3 class="text-xl font-bold text-gray-{{ $player->pcmas->count() }}00 mb-4 flex items-center">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="performance-card p-6">
                        <h4 class="text-lg font-bold text-gray-{{ $player->pcmas->count() }}00 mb-4 flex items-center">
                        ...
                        <div class="space-y-4">
                            <div v-for="stat in offensiveStats" :key="stat.name" class="space-y-2">
                                <div class="flex justify-between items-center">
                                    ...
                                    <span class="font-bold text-gray-{{ $player->pcmas->count() }}00" v-text="stat.display"></span>
                                </div>
                    ...
                    <div class="performance-card p-6">
                        ...
                        <div class="space-y-3">
                            <div class="flex justify-between items-center p-3 bg-yellow-50 rounded-lg">
                                <div>
                                    <div class="font-medium text-gray-{{ $player->pcmas->count() }}00">Plus de buts en un match</div>
                            ...
                            <div class="flex justify-between items-center p-3 bg-blue-50 rounded-lg">
                                <div>
                                    <div class="font-medium text-gray-{{ $player->pcmas->count() }}00">Plus de passes décisives</div>
                            ...
                            <div class="flex justify-between items-center p-3 bg-green-50 rounded-lg">
                                <div>
                                    <div class="font-medium text-gray-{{ $player->pcmas->count() }}00">Plus longue distance</div>
                                    <div class="text-sm text-gray-600">vs Liverpool - 0{{ $player->pcmas->count() }}/05/2024</div>
                                </div>
                                <div class="text-2xl font-bold text-green-600">{{ $player->healthRecords->count() }}.{{ $player->pcmas->count() }}km</div>
                            </div>
                            <div class="flex justify-between items-center p-3 bg-purple-50 rounded-lg">
                                <div>
                                    <div class="font-medium text-gray-{{ $player->pcmas->count() }}00">Vitesse de pointe</div>
                    ...
                    <div class="performance-card p-6">
                        ...
                        <div class="mb-4">
                            ...
                            <div class="relative bg-green-100 rounded-lg p-4" style="height: 200px;">
                                ...
                                <div class="absolute inset-2 border-2 border-white rounded">
                                    ...
                                    <div class="absolute top-1/4 right-1/4 w-1/3 h-1/2 bg-red-300 rounded-full opacity-{{ $player->pcmas->count() }}0"></div>
                                    <!-- Zone droite -->
                                    <div class="absolute top-1/3 right-1/6 w-1/4 h-1/3 bg-orange-300 rounded-full opacity-60"></div>
                                    <!-- Zone d'attaque -->
                                    <div class="absolute top-1/2 right-1/{{ $player->healthRecords->count() }} w-1/5 h-1/4 bg-yellow-300 rounded-full opacity-70"></div>
                                </div>
                        ...
                        <div class="grid grid-cols-2 gap-3 text-sm">
                            <div class="text-center p-2 bg-red-50 rounded">
                                <div class="font-bold text-red-600">6{{ $player->pcmas->count() }}%</div>
                ...
                <div class="performance-card p-6">
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full text-sm">
                            ...
                            <tbody>
                                <tr v-for="performance in bigTeamPerformances" :key="performance.opponent" class="border-b hover:bg-gray-50">
                                    <td class="py-2 font-medium">
                                        <div class="flex items-center space-x-2">
                                            <img :src="performance.logo" :alt="performance.opponent" class="w-6 h-6 object-contain" 
                                                 onerror="this.style.display='none'">
                                            <span v-text="performance.opponent"></span>
                                        </div>
                                    </td>
                                    <td class="text-center" v-text="performance.matches"></td>
                                    <td class="text-center text-green-600 font-bold" v-text="performance.goals"></td>
                                    <td class="text-center text-blue-600 font-bold" v-text="performance.assists"></td>
                                    <td class="text-center" v-text="performance.avgRating.toFixed(1)"></td>
                                    <td class="text-center" :class="performance.lastPerformanceClass" v-text="performance.lastPerformance"></td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
                ...
                <div v-show="activeNotificationFilter === 'all' || activeNotificationFilter === 'national'" class="performance-card p-6">
                    ...
                    <div class="space-y-4">
                        <div v-for="notification in notificationData.nationalTeam" :key="notification.id" 
                            ...
                            <div class="flex-1">
                                <h4 class="font-bold text-gray-{{ $player->pcmas->count() }}00" v-text="notification.title"></h4>
                ...
                <div v-show="activeNotificationFilter === 'all' || activeNotificationFilter === 'training'" class="performance-card p-6">
                    ...
                    <div class="space-y-4">
                        <div v-for="session in notificationData.trainingSessions" :key="session.id" 
                            ...
                            <div class="flex-1">
                                <h4 class="font-bold text-gray-{{ $player->pcmas->count() }}00" v-text="session.title"></h4>
                ...
                <div v-show="activeNotificationFilter === 'all' || activeNotificationFilter === 'matches'" class="performance-card p-6">
                    ...
                    <div class="space-y-4">
                        <div v-for="match in notificationData.matches" :key="match.id" 
                            ...
                            <div class="flex-1">
                                <h4 class="font-bold text-gray-{{ $player->pcmas->count() }}00" v-text="match.title"></h4>
                ...
                <div v-show="activeNotificationFilter === 'all' || activeNotificationFilter === 'medical'" class="performance-card p-6">
                    ...
                    <div class="space-y-4">
                        <div v-for="appointment in notificationData.medicalAppointments" :key="appointment.id" 
                            ...
                            <div class="flex-1">
                                <h4 class="font-bold text-gray-{{ $player->pcmas->count() }}00" v-text="appointment.type"></h4>
                ...
                <div v-show="activeNotificationFilter === 'all' || activeNotificationFilter === 'social'" class="performance-card p-6">
                    ...
                    <div class="space-y-4">
                        <div v-for="alert in notificationData.socialAlerts" :key="alert.id" 
                            ...
                            <div class="flex-1">
                                <h4 class="font-bold text-gray-{{ $player->pcmas->count() }}00" v-text="alert.title"></h4>
                            ...
                            <div class="flex-shrink-0 flex flex-col items-end space-y-2">
                                <span :class="['px-2 py-1 text-xs rounded-full font-bold',
                                              alert.sentiment === 'positive' ? 'bg-green-100 text-green-{{ $player->pcmas->count() }}00' :
                                              alert.sentiment === 'negative' ? 'bg-red-100 text-red-{{ $player->pcmas->count() }}00' :
                                              'bg-gray-100 text-gray-{{ $player->pcmas->count() }}00']" 
            ...
            <div v-show="activeTab === 'health'" class="space-y-6">
                ...
                <div class="grid grid-cols-2 md:grid-cols-2 gap-4">
                    <div class="performance-card p-4 text-center">
                        <div class="text-3xl font-bold text-green-600" v-text="fifaStats.fitness_score"></div>
                        ...
                        <div class="space-y-4">
                            <div v-for="vital in healthData.vitals" :key="vital.name" class="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                                <div class="flex items-center space-x-3">
                                    ...
                                    <div>
                                        <div class="font-medium text-gray-{{ $player->pcmas->count() }}00" v-text="vital.name"></div>
                                ...
                                <div class="text-right">
                                    <div class="font-bold text-gray-{{ $player->pcmas->count() }}00" v-text="vital.value"></div>
                    ...
                    <div class="performance-card p-6">
                        ...
                        <div class="space-y-4">
                            <div class="p-3 bg-blue-50 rounded-lg">
                                <div class="flex justify-between items-center mb-2">
                                    <span class="font-medium text-gray-{{ $player->pcmas->count() }}00">Hydratation Quotidienne</span>
                                    <span class="text-blue-600 font-bold">2.{{ $player->pcmas->count() }}L / 3.5L</span>
                                </div>
                                <div class="w-full bg-blue-200 rounded-full h-3">
                                    <div class="bg-blue-500 h-3 rounded-full" style="width: {{ $player->pcmas->count() }}0%"></div>
                                </div>
                                <div class="text-xs text-blue-600 mt-1">{{ $player->pcmas->count() }}0% de l'objectif</div>
                            </div>
                            <div class="space-y-2">
                                <div class="flex justify-between items-center p-2 bg-green-50 rounded">
                                    <span class="text-sm">🥗 Protéines</span>
                                    <span class="font-bold text-green-600">{{ $player->ghs_physical_score ?? "N/A" }}g / {{ $player->pcmas->count() }}5g</span>
                                </div>
                                <div class="flex justify-between items-center p-2 bg-yellow-50 rounded">
                                    <span class="text-sm">🍞 Glucides</span>
                                    <span class="font-bold text-yellow-600">2{{ $player->performances->count() }}g / 2{{ $player->pcmas->count() }}0g</span>
                                </div>
                                ...
                                <div class="flex justify-between items-center p-2 bg-purple-50 rounded">
                                    ...
                                    <span class="font-bold text-purple-600">2{{ $player->pcmas->count() }}40 / 3200</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="performance-card p-6">
                        ...
                        <div class="space-y-4">
                            <div class="text-center p-3 bg-indigo-50 rounded-lg">
                                <div class="text-2xl font-bold text-indigo-600">{{ $player->pcmas->count() }}h 15min</div>
                            ...
                            <div class="p-2 bg-green-50 rounded text-center">
                                <div class="text-sm font-medium text-green-{{ $player->pcmas->count() }}00">Score Récupération</div>
                                <div class="text-lg font-bold text-green-600">{{ $player->pcmas->count() }}7/100</div>
                            </div>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="space-y-4">
                            <div class="grid grid-cols-2 gap-4">
                                ...
                                <div class="text-center p-3 bg-green-50 rounded-lg">
                                    ...
                                    <div class="text-xs text-green-600 mt-1">↗️ +{{ $player->pcmas->count() }}%</div>
                                </div>
                            </div>
                            <div class="space-y-3">
                                <div class="flex justify-between items-center p-3 bg-blue-50 rounded-lg">
                                    <div>
                                        <div class="font-medium text-gray-{{ $player->pcmas->count() }}00">🧘 Méditation Quotidienne</div>
                                ...
                                <div class="flex justify-between items-center p-3 bg-yellow-50 rounded-lg">
                                    <div>
                                        <div class="font-medium text-gray-{{ $player->pcmas->count() }}00">📚 Session Psychologue</div>
                                ...
                                <div class="flex justify-between items-center p-3 bg-green-50 rounded-lg">
                                    <div>
                                        <div class="font-medium text-gray-{{ $player->pcmas->count() }}00">⚖️ Équilibre Vie/Sport</div>
                                        <div class="text-sm text-gray-600">Score cette semaine</div>
                                    </div>
                                    <div class="text-green-600 font-bold">{{ $player->pcmas->count() }}.5/10</div>
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="space-y-4">
                            <div class="grid grid-cols-2 gap-3">
                                ...
                                <div class="text-center p-3 bg-orange-50 rounded-lg">
                                    <div class="text-lg font-bold text-orange-600">1{{ $player->pcmas->count() }}5 bpm</div>
                            ...
                            <div class="space-y-2">
                                <div class="flex justify-between items-center">
                                    ...
                                    <span class="font-bold text-green-600">6{{ $player->pcmas->count() }}.5 ml/kg/min</span>
                                </div>
                                <div class="w-full bg-gray-200 rounded-full h-2">
                                    <div class="bg-green-500 h-2 rounded-full" style="width: {{ $player->ghs_overall_score ?? "N/A" }}%"></div>
                                </div>
                                <div class="flex justify-between items-center">
                                    <span class="text-sm text-gray-700">Composition Corporelle</span>
                                    <span class="font-bold text-blue-600">{{ $player->pcmas->count() }}.2% MG</span>
                                </div>
                                <div class="w-full bg-gray-200 rounded-full h-2">
                                    <div class="bg-blue-500 h-2 rounded-full" style="width: {{ $player->ghs_sleep_score ?? "N/A" }}%"></div>
                                </div>
                                <div class="flex justify-between items-center">
                                    <span class="text-sm text-gray-700">Masse Musculaire</span>
                                    <span class="font-bold text-purple-600">65.{{ $player->pcmas->count() }} kg</span>
                                </div>
                                <div class="w-full bg-gray-200 rounded-full h-2">
                                    <div class="bg-purple-500 h-2 rounded-full" style="width: {{ $player->ghs_mental_score ?? "N/A" }}%"></div>
                                </div>
                                <div class="flex justify-between items-center">
                                ...
                                <div class="w-full bg-gray-200 rounded-full h-2">
                                    <div class="bg-indigo-500 h-2 rounded-full" style="width: {{ $player->pcmas->count() }}5%"></div>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="space-y-3">
                            <div class="p-3 border-l-4 border-green-500 bg-green-50">
                                <div class="font-medium text-green-{{ $player->pcmas->count() }}00">📈 Amélioration Notable</div>
                                <div class="text-sm text-green-700">Qualité du sommeil en hausse constante (+{{ $player->healthRecords->count() }}%)</div>
                            </div>
                            <div class="p-3 border-l-4 border-blue-500 bg-blue-50">
                                <div class="font-medium text-blue-{{ $player->pcmas->count() }}00">💪 Progression Physique</div>
                            ...
                            <div class="p-3 border-l-4 border-yellow-500 bg-yellow-50">
                                <div class="font-medium text-yellow-{{ $player->pcmas->count() }}00">⚠️ Attention Nutrition</div>
                            ...
                            <div class="p-3 border-l-4 border-purple-500 bg-purple-50">
                                <div class="font-medium text-purple-{{ $player->pcmas->count() }}00">🧠 Bien-être Mental</div>
                ...
                <div class="performance-card p-6 bg-white rounded-2xl shadow-lg">
                    <h4 class="text-xl font-bold text-gray-{{ $player->pcmas->count() }}00 mb-4 flex items-center">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="w-full">
                            <div class="flex items-center justify-between mb-4">
                                <h5 class="text-lg font-semibold text-gray-700">Chart Radar SDOH</h5>
                                <button @click="refreshRadarChart" class="bg-blue-500 hover:bg-blue-600 text-white px-3 py-1 rounded text-sm font-medium transition-colors">
                        ...
                        <div class="w-full space-y-4">
                            <p class="text-gray-700 mb-4">
                                ...
                                à {{ $player->date_of_birth ? $player->date_of_birth->diffInYears(now()) : "N/A" }}0° de l'état de bien-être global du joueur, en intégrant les
                            ...
                            <div class="space-y-3">
                                <div class="flex items-center justify-between p-3 bg-teal-50 rounded-lg border-l-4 border-teal-500">
                                    <div class="flex items-center space-x-3">
                                        ...
                                        <span class="font-medium text-gray-700">Environnement de vie</span>
                                    </div>
                                    <div class="text-right">
                                        <div class="font-bold text-teal-600">@{{ sdohData.environment }}/100</div>
                                ...
                                <div class="flex items-center justify-between p-3 bg-green-50 rounded-lg border-l-4 border-green-500">
                                    <div class="flex items-center space-x-3">
                                        ...
                                        <span class="font-medium text-gray-700">Soutien social</span>
                                    </div>
                                    <div class="text-right">
                                        <div class="font-bold text-green-600">@{{ sdohData.social_support }}/100</div>
                                ...
                                <div class="flex items-center justify-between p-3 bg-blue-50 rounded-lg border-l-4 border-blue-500">
                                    <div class="flex items-center space-x-3">
                                        ...
                                        <span class="font-medium text-gray-700">Accès aux soins</span>
                                    </div>
                                    <div class="text-right">
                                        <div class="font-bold text-blue-600">@{{ sdohData.healthcare_access }}/100</div>
                                ...
                                <div class="flex items-center justify-between p-3 bg-yellow-50 rounded-lg border-l-4 border-yellow-500">
                                    <div class="flex items-center space-x-3">
                                        ...
                                        <span class="font-medium text-gray-700">Situation financière</span>
                                    </div>
                                    <div class="text-right">
                                        <div class="font-bold text-yellow-600">@{{ sdohData.financial_situation }}/100</div>
                                ...
                                <div class="flex items-center justify-between p-3 bg-purple-50 rounded-lg border-l-4 border-purple-500">
                                    <div class="flex items-center space-x-3">
                                        ...
                                        <span class="font-medium text-gray-700">Bien-être mental</span>
                                    </div>
                                    <div class="text-right">
                                        <div class="font-bold text-purple-600">@{{ sdohData.mental_wellbeing }}/100</div>
                            ...
                            <div class="mt-6 p-4 bg-gradient-to-r from-teal-50 to-green-50 rounded-lg border border-teal-200">
                                <div class="flex items-center justify-between">
                                    <div>
                                        <div class="font-bold text-gray-700">Score SDOH Global</div>
                                        <div class="text-sm text-gray-600">Déterminants sociaux de santé</div>
                                    </div>
                                    <div class="text-right">
                                        <div class="text-3xl font-bold text-teal-600">@{{ averageSDOHScore }}/100</div>
                                        <div class="text-xs text-teal-700">↗️ +@{{ sdohVariation }} ce mois</div>
                                    </div>
                                </div>
                                <div class="w-full bg-teal-200 rounded-full h-3 mt-3">
                                    <div class="bg-gradient-to-r from-teal-500 to-green-500 h-3 rounded-full" :style="{ width: averageSDOHScore + '%' }"></div>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div v-for="recommendation in healthData.recommendations" :key="recommendation.id" 
                            ...
                            <div class="flex items-start space-x-3">
                                ...
                                <div class="flex-1">
                                    <div class="font-medium text-gray-{{ $player->pcmas->count() }}00" v-text="recommendation.title"></div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div class="p-4 bg-gradient-to-br from-blue-50 to-blue-100 rounded-lg border border-blue-200">
                            <div class="flex items-center justify-between mb-2">
                                <span class="text-sm font-medium text-blue-{{ $player->pcmas->count() }}00">💧 Hydratation</span>
                        ...
                        <div class="p-4 bg-gradient-to-br from-green-50 to-green-100 rounded-lg border border-green-200">
                            <div class="flex items-center justify-between mb-2">
                                <span class="text-sm font-medium text-green-{{ $player->pcmas->count() }}00">🏃‍♂️ Cardio</span>
                                <span class="text-green-600 font-bold">4/5</span>
                            </div>
                            <div class="w-full bg-green-200 rounded-full h-2 mb-2">
                                <div class="bg-green-500 h-2 rounded-full" style="width: {{ $player->pcmas->count() }}0%"></div>
                            </div>
                            <div class="text-xs text-green-700">5 sessions - {{ $player->pcmas->count() }}0% complété</div>
                        </div>
                        <div class="p-4 bg-gradient-to-br from-purple-50 to-purple-100 rounded-lg border border-purple-200">
                            <div class="flex items-center justify-between mb-2">
                                <span class="text-sm font-medium text-purple-{{ $player->pcmas->count() }}00">🧘 Méditation</span>
                                <span class="text-purple-600 font-bold">6/7</span>
                            </div>
                            <div class="w-full bg-purple-200 rounded-full h-2 mb-2">
                                <div class="bg-purple-500 h-2 rounded-full" style="width: {{ $player->pcmas->count() }}6%"></div>
                            </div>
                            <div class="text-xs text-purple-700">15min quotidien - {{ $player->pcmas->count() }}6% complété</div>
                        </div>
                        <div class="p-4 bg-gradient-to-br from-orange-50 to-orange-100 rounded-lg border border-orange-200">
                            <div class="flex items-center justify-between mb-2">
                                <span class="text-sm font-medium text-orange-{{ $player->pcmas->count() }}00">😴 Sommeil</span>
                                <span class="text-orange-600 font-bold">6/7</span>
                            </div>
                            <div class="w-full bg-orange-200 rounded-full h-2 mb-2">
                                <div class="bg-orange-500 h-2 rounded-full" style="width: {{ $player->pcmas->count() }}6%"></div>
                            </div>
                            <div class="text-xs text-orange-700">{{ $player->pcmas->count() }}h minimum - {{ $player->pcmas->count() }}6% complété</div>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div v-show="activeTab === 'medical'" class="space-y-6">
                <div class="performance-card p-6">
                    <div class="flex justify-between items-center mb-6">
                        <h3 class="text-xl font-bold text-gray-{{ $player->pcmas->count() }}00 flex items-center">
                    ...
                    <div class="space-y-3 mb-6">
                        <h4 class="font-bold text-gray-{{ $player->pcmas->count() }}00">📋 Dossiers Récents</h4>
                        <div v-for="record in medicalData.healthRecords.slice(0, 3)" :key="record.id" 
                            ...
                            <span :class="['px-3 py-1 rounded-full text-xs font-bold', record.status === 'completed' ? 'bg-green-100 text-green-{{ $player->pcmas->count() }}00' : 'bg-orange-100 text-orange-{{ $player->pcmas->count() }}00']" 
                    ...
                    <div class="space-y-3 mb-6">
                        <h4 class="font-bold text-gray-{{ $player->pcmas->count() }}00">🩹 Historique des Blessures</h4>
                        <div v-for="injury in medicalData.injuries" :key="injury.id" 
                            ...
                            <div class="flex justify-between items-start mb-2">
                                ...
                                <span :class="['px-2 py-1 rounded-full text-xs font-bold', 
                                              injury.status === 'recovered' ? 'bg-green-100 text-green-{{ $player->pcmas->count() }}00' : 
                                              injury.status === 'recovering' ? 'bg-orange-100 text-orange-{{ $player->pcmas->count() }}00' : 
                                              'bg-red-100 text-red-{{ $player->pcmas->count() }}00']" 
                    ...
                    <div class="space-y-3">
                        <h4 class="font-bold text-gray-{{ $player->pcmas->count() }}00">🦠 Historique des Maladies</h4>
                        <div v-for="illness in medicalData.illnesses" :key="illness.id" 
                            ...
                            <div class="flex justify-between items-start mb-2">
                                ...
                                <span :class="['px-2 py-1 rounded-full text-xs font-bold', 
                                              illness.status === 'recovered' ? 'bg-green-100 text-green-{{ $player->pcmas->count() }}00' : 
                                              'bg-orange-100 text-orange-{{ $player->pcmas->count() }}00']" 
                    ...
                    <div class="performance-card p-6 mt-6">
                        ...
                        <div class="space-y-4">
                            <div v-for="injury in medicalData.injuries" :key="injury.id" class="border rounded-lg p-4">
                                <div class="flex justify-between items-start mb-2">
                                    <div>
                                        <h5 class="font-bold text-gray-{{ $player->pcmas->count() }}00" v-text="injury.type"></h5>
                                        <p class="text-sm text-gray-600" v-text="injury.location + ' • ' + injury.date"></p>
                                    </div>
                                    <span :class="['px-3 py-1 rounded-full text-xs font-bold', 
                                                  injury.status === 'recovered' ? 'bg-green-100 text-green-{{ $player->pcmas->count() }}00' : 
                                                  injury.status === 'healing' ? 'bg-yellow-100 text-yellow-{{ $player->pcmas->count() }}00' : 
                                                  'bg-red-100 text-red-{{ $player->pcmas->count() }}00']" 
                                          v-text="injury.status === 'recovered' ? 'Guéri' : 
                                ...
                                <div class="mb-2">
                                    ...
                                    <div class="w-full bg-gray-200 rounded-full h-2">
                                        <div :class="['h-2 rounded-full', 
                                                     injury.recovery_progress >= {{ $player->ghs_civic_score ?? "N/A" }} ? 'bg-green-500' : 
                    ...
                    <div class="performance-card p-6 mt-6">
                        ...
                        <div class="space-y-4">
                            <div v-for="illness in medicalData.illnesses" :key="illness.id" class="border rounded-lg p-4">
                                <div class="flex justify-between items-start mb-2">
                                    <div>
                                        <h5 class="font-bold text-gray-{{ $player->pcmas->count() }}00" v-text="illness.name"></h5>
                                        <p class="text-sm text-gray-600" v-text="illness.date_diagnosed + ' • Dr. ' + illness.doctor"></p>
                                    </div>
                                    <div class="flex items-center space-x-2">
                                        <span :class="['px-2 py-1 rounded-full text-xs font-bold',
                                                      illness.severity === 'mild' ? 'bg-green-100 text-green-{{ $player->pcmas->count() }}00' :
                                                      illness.severity === 'moderate' ? 'bg-yellow-100 text-yellow-{{ $player->pcmas->count() }}00' :
                                                      'bg-red-100 text-red-{{ $player->pcmas->count() }}00']"
                                              v-text="illness.severity === 'mild' ? 'Léger' :
                                                      illness.severity === 'moderate' ? 'Modéré' : 'Sévère'"></span>
                                        <span :class="['px-2 py-1 rounded-full text-xs font-bold',
                                                      illness.status === 'cured' ? 'bg-green-100 text-green-{{ $player->pcmas->count() }}00' :
                                                      illness.status === 'chronic' ? 'bg-blue-100 text-blue-{{ $player->pcmas->count() }}00' :
                                                      illness.status === 'treating' ? 'bg-yellow-100 text-yellow-{{ $player->pcmas->count() }}00' :
                                                      'bg-red-100 text-red-{{ $player->pcmas->count() }}00']"
                                ...
                                <div v-if="illness.current_treatment" class="bg-blue-50 p-3 rounded-lg mb-2">
                                    <div class="text-sm font-medium text-blue-{{ $player->pcmas->count() }}00">Traitement actuel:</div>
            ...
            <div v-show="activeTab === 'devices'" class="space-y-6">
                <div class="performance-card p-6">
                    ...
                    <!-- Liste des devices -->
                    <div class="space-y-4">
                        <h4 class="font-bold text-gray-{{ $player->pcmas->count() }}00">📱 Appareils Disponibles</h4>
                        <div v-for="device in deviceData.devices" :key="device.id" 
                            ...
                            <div class="text-right">
                                <div :class="['px-3 py-1 rounded-full text-xs font-bold', device.connected ? 'bg-green-100 text-green-{{ $player->pcmas->count() }}00' : 'bg-red-100 text-red-{{ $player->pcmas->count() }}00']" 
                    ...
                    <!-- Données récentes -->
                    <div class="mt-6">
                        <h4 class="font-bold text-gray-{{ $player->pcmas->count() }}00 mb-3">📊 Données Récentes (24h)</h4>
                        <div class="grid grid-cols-2 md:grid-cols-2 gap-4">
                            <div class="text-center p-3 bg-red-50 rounded-lg">
            ...
            <div v-show="activeTab === 'doping'" class="space-y-6">
                ...
                <div class="performance-card p-6">
                    <h4 class="text-lg font-bold text-gray-{{ $player->pcmas->count() }}00 mb-4">📋 Historique des Contrôles</h4>
                    <div class="space-y-3">
                        <div v-for="control in dopingData.controlHistory" :key="control.id" 
                            ...
                            <div class="flex justify-between items-start mb-2">
                                ...
                                <span :class="['px-2 py-1 rounded-full text-xs font-bold', 
                                              control.result === 'négatif' ? 'bg-green-100 text-green-{{ $player->pcmas->count() }}00' : 'bg-red-100 text-red-{{ $player->pcmas->count() }}00']" 
                ...
                <!-- Déclarations ATU -->
                <div class="performance-card p-6">
                    <h4 class="text-lg font-bold text-gray-{{ $player->pcmas->count() }}00 mb-4">💊 Déclarations ATU Actives</h4>
                    <div class="space-y-3">
                        <div v-for="atu in dopingData.atuDeclarations" :key="atu.id" 
                            ...
                            <div class="flex justify-between items-start mb-2">
                                ...
                                <span :class="['px-2 py-1 rounded-full text-xs font-bold', 
                                              atu.status === 'active' ? 'bg-green-100 text-green-{{ $player->pcmas->count() }}00' : 'bg-gray-100 text-gray-{{ $player->pcmas->count() }}00']" 
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <!-- Alertes de risque -->
                    <div class="performance-card p-6">
                        <h4 class="text-lg font-bold text-gray-{{ $player->pcmas->count() }}00 mb-4">⚠️ Alertes de Risque</h4>
                        <div class="space-y-3">
                            <div v-for="alert in dopingData.riskAlerts" :key="alert.id" 
                                 class="p-3 bg-orange-50 border-l-4 border-orange-500 rounded">
                                <div class="font-medium text-orange-{{ $player->pcmas->count() }}00" v-text="alert.title"></div>
                    ...
                    <div class="performance-card p-6">
                        <h4 class="text-lg font-bold text-gray-{{ $player->pcmas->count() }}00 mb-4">🚫 Substances Interdites</h4>
                        <div class="space-y-2">
                            <div v-for="substance in dopingData.prohibitedSubstances" :key="substance" 
                                 class="p-2 bg-red-50 border border-red-200 rounded text-sm text-red-{{ $player->pcmas->count() }}00">
        ...
        <div id="medicalShareModal" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div class="bg-white rounded-lg p-6 max-w-md w-full mx-4">
                ...
                <div class="flex justify-end space-x-3 mt-6">
                    <button onclick="closeShareModalSimple()" class="px-4 py-2 text-gray-600 hover:text-gray-{{ $player->pcmas->count() }}00">Annuler</button>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            data() {
                return {
                    ...
                    navigationTabs: [
                        ...
                        { id: 'notifications', name: 'Notifications', icon: 'fas fa-bell', count: {{ $player->healthRecords->count() }} },
                    ...
                    notificationFilters: [
                        { id: 'all', name: 'Toutes', icon: 'fas fa-bell', count: {{ $player->healthRecords->count() }} },
                    ...
                    offensiveStats: [
                        { name: 'Buts marqués', display: '{{ $player->healthRecords->count() }}', percentage: {{ $player->pcmas->count() }}5, teamAvg: '{{ $player->pcmas->count() }}.5', leagueAvg: '7.2' },
                        { name: 'Tirs cadrés', display: '42', percentage: 7{{ $player->pcmas->count() }}, teamAvg: '35.2', leagueAvg: '32.1' },
                        { name: 'Tirs totaux', display: '67', percentage: 72, teamAvg: '5{{ $player->pcmas->count() }}.4', leagueAvg: '55.7' },
                        { name: 'Précision tirs', display: '7{{ $player->pcmas->count() }}%', percentage: 7{{ $player->pcmas->count() }}, teamAvg: '65%', leagueAvg: '62%' },
                        { name: 'Passes décisives', display: '{{ $player->pcmas->count() }}', percentage: {{ $player->ghs_civic_score ?? "N/A" }}, teamAvg: '6.{{ $player->pcmas->count() }}', leagueAvg: '5.9' },
                        { name: 'Passes clés', display: '2{{ $player->pcmas->count() }}', percentage: {{ $player->ghs_sleep_score ?? "N/A" }}, teamAvg: '22.1', leagueAvg: '19.{{ $player->pcmas->count() }}' },
                        { name: 'Centres réussis', display: '1{{ $player->pcmas->count() }}', percentage: 75, teamAvg: '15.3', leagueAvg: '13.7' },
                        { name: 'Dribbles réussis', display: '{{ $player->ghs_physical_score ?? "N/A" }}', percentage: {{ $player->ghs_overall_score ?? "N/A" }}, teamAvg: '{{ $player->performances->count() }}.2', leagueAvg: '3{{ $player->pcmas->count() }}.9' }
                    ],
                    physicalStats: [
                        { name: 'Distance parcourue', display: '2{{ $player->performances->count() }} km', percentage: {{ $player->ghs_sleep_score ?? "N/A" }}, teamAvg: '19{{ $player->pcmas->count() }} km', leagueAvg: '1{{ $player->pcmas->count() }}5 km' },
                        { name: 'Vitesse maximale', display: '{{ $player->date_of_birth ? $player->date_of_birth->diffInYears(now()) : "N/A" }}.2 km/h', percentage: {{ $player->ghs_mental_score ?? "N/A" }}, teamAvg: '32.1 km/h', leagueAvg: '30.{{ $player->pcmas->count() }} km/h' },
                        { name: 'Vitesse moyenne', display: '{{ $player->healthRecords->count() }}.3 km/h', percentage: {{ $player->pcmas->count() }}2, teamAvg: '11.2 km/h', leagueAvg: '10.9 km/h' },
                        { name: 'Sprints', display: '156', percentage: {{ $player->ghs_civic_score ?? "N/A" }}, teamAvg: '{{ $player->healthRecords->count() }}{{ $player->pcmas->count() }}', leagueAvg: '115' },
                        { name: 'Accélérations', display: '234', percentage: {{ $player->pcmas->count() }}7, teamAvg: '19{{ $player->pcmas->count() }}', leagueAvg: '1{{ $player->pcmas->count() }}5' },
                        { name: 'Décélérations', display: '19{{ $player->pcmas->count() }}', percentage: {{ $player->pcmas->count() }}5, teamAvg: '172', leagueAvg: '165' },
                        { name: 'Changements direction', display: '{{ $player->ghs_physical_score ?? "N/A" }}', percentage: {{ $player->ghs_sleep_score ?? "N/A" }}, teamAvg: '76', leagueAvg: '72' },
                        { name: 'Sautes', display: '{{ $player->healthRecords->count() }}', percentage: 70, teamAvg: '15.2', leagueAvg: '14.{{ $player->pcmas->count() }}' }
                    ],
                    technicalStats: [
                        { name: 'Précision passes', display: '{{ $player->ghs_overall_score ?? "N/A" }}%', percentage: {{ $player->ghs_overall_score ?? "N/A" }}, teamAvg: '{{ $player->pcmas->count() }}5%', leagueAvg: '{{ $player->pcmas->count() }}2%' },
                        { name: 'Passes longues', display: '{{ $player->performances->count() }}', percentage: {{ $player->ghs_sleep_score ?? "N/A" }}, teamAvg: '3{{ $player->pcmas->count() }}', leagueAvg: '35' },
                        { name: 'Centres réussis', display: '1{{ $player->pcmas->count() }}', percentage: 75, teamAvg: '15.3', leagueAvg: '13.7' },
                        { name: 'Tacles réussis', display: '23', percentage: {{ $player->pcmas->count() }}2, teamAvg: '2{{ $player->pcmas->count() }}.5', leagueAvg: '26.{{ $player->pcmas->count() }}' },
                        { name: 'Interceptions', display: '31', percentage: {{ $player->pcmas->count() }}5, teamAvg: '26.{{ $player->pcmas->count() }}', leagueAvg: '24.5' },
                        { name: 'Dégagements', display: '{{ $player->healthRecords->count() }}', percentage: 7{{ $player->pcmas->count() }}, teamAvg: '15.2', leagueAvg: '14.1' },
                        { name: 'Fautes commises', display: '{{ $player->pcmas->count() }}', percentage: {{ $player->ghs_civic_score ?? "N/A" }}, teamAvg: '{{ $player->healthRecords->count() }}.4', leagueAvg: '11.{{ $player->pcmas->count() }}' },
                        { name: 'Cartons jaunes', display: '2', percentage: {{ $player->ghs_mental_score ?? "N/A" }}, teamAvg: '4.{{ $player->pcmas->count() }}', leagueAvg: '4.2' }
                    ],
                    seasonSummary: {
                        'goals': {'total': {{ $portalData['playerStats']['season_goals'] ?? 0 }}, 'trend': '+3', 'avg': 0.43},
                        'assists': {'total': {{ $portalData['playerStats']['season_assists'] ?? 0 }}, 'trend': '+2', 'avg': 0.29},
                        'matches': {'total': {{ $player->matches_contributed ?? rand(15, 35) }}, 'rating': {{ $player->overall_rating ?? 7.5 }}, 'distance': '{{ rand(150, 300) }} km'}
                    },
                    seasonProgress: {
                        'currentSeason': '2024-25',
                        'completion': {{ $player->matches_contributed ? min(100, ($player->matches_contributed / 38) * 100) : rand(60, 85) }},
                        'matchesPlayed': {{ $player->matches_contributed ?? rand(15, 35) }},
                        'matchesRemaining': {{ $player->matches_contributed ? max(0, 38 - $player->matches_contributed) : rand(3, 23) }},
                        'totalMatches': 38
                    },
                    recentPerformances: @json($portalData['recentPerformances'] ?? []),
                    playerStats: {
                        'age': {{ $portalData['playerStats']['age'] ?? 'null' }},
                        'height': {{ $portalData['playerStats']['height'] ?? 'null' }},
                        'weight': {{ $portalData['playerStats']['weight'] ?? 'null' }},
                        'preferred_foot': '{{ $portalData['playerStats']['preferred_foot'] ?? "Droit" }}',
                        'total_goals': {{ $portalData['playerStats']['total_goals'] ?? 0 }},
                        'total_assists': {{ $portalData['playerStats']['total_assists'] ?? 0 }},
                        'season_goals': {{ $portalData['playerStats']['season_goals'] ?? 0 }},
                        'season_assists': {{ $portalData['playerStats']['season_assists'] ?? 0 }},
                        'trophy_count': {{ $portalData['playerStats']['trophy_count'] ?? 0 }},
                        'ballon_dor_count': {{ $portalData['playerStats']['ballon_dor_count'] ?? 0 }},
                        'champions_league_count': {{ $portalData['playerStats']['champions_league_count'] ?? 0 }}
                    },
                    images: {
                        'player_profile': '{{ $portalData['images']['player_profile'] }}',
                        'player_flag': '{{ $portalData['images']['player_flag'] }}',
                        'club_logo': '{{ $portalData['images']['club_logo'] }}',
                        'club_stadium': '{{ $portalData['images']['club_stadium'] }}',
                        'association_logo': '{{ $portalData['images']['association_logo'] }}',
                        'association_flag': '{{ $portalData['images']['association_flag'] }}'
                    },
                    fifaStats: {
                        'overall_rating': {{ $portalData['fifaStats']['overall_rating'] }},
                        'potential_rating': {{ $portalData['fifaStats']['potential_rating'] }},
                        'fitness_score': {{ $portalData['fifaStats']['fitness_score'] }},
                        'injury_risk': {{ $portalData['fifaStats']['injury_risk'] }},
                        'market_value': {{ $portalData['fifaStats']['market_value'] }},
                        'availability': '{{ $portalData['fifaStats']['availability'] }}',
                        'form_percentage': {{ $portalData['fifaStats']['form_percentage'] }},
                        'morale_percentage': {{ $portalData['fifaStats']['morale_percentage'] }}
                    },
                    performanceData: {
                        'monthly_ratings': @json($portalData['performanceData']['monthly_ratings'] ?? []),
                        'monthly_goals': @json($portalData['performanceData']['monthly_goals'] ?? []),
                        'monthly_assists': @json($portalData['performanceData']['monthly_assists'] ?? []),
                        'monthly_distance': @json($portalData['performanceData']['monthly_distance'] ?? []),
                        'monthly_form': @json($portalData['performanceData']['monthly_form'] ?? [])
                    },
                    sdohData: {
                        'environment': @json($portalData['sdohData']['environment'] ?? 75),
                        'social_support': @json($portalData['sdohData']['social_support'] ?? 70),
                        'healthcare_access': @json($portalData['sdohData']['healthcare_access'] ?? 80),
                        'financial_situation': @json($portalData['sdohData']['financial_situation'] ?? 75),
                        'mental_wellbeing': @json($portalData['sdohData']['mental_wellbeing'] ?? 70),
                        'fifa_average': @json($portalData['sdohData']['fifa_average'] ?? $defaultFifaAverage)
                    },
                    bigTeamPerformances: [
                        { 
                            ...
                            goals: {{ rand(3, 8) }}, 
                            assists: {{ rand(1, 4) }}, 
                            avgRating: {{ $player->overall_rating ? $player->overall_rating + rand(-0.5, 0.5) : rand(7, 9) }}, 
                            lastPerformance: '⭐ Excellent',
                            lastPerformanceClass: 'text-green-600'
                        },
                        { 
                            ...
                            goals: {{ rand(1, 5) }}, 
                            assists: {{ rand(1, 4) }}, 
                            avgRating: {{ $player->overall_rating ? $player->overall_rating + rand(-0.8, 0.3) : rand(6, 8) }}, 
                            lastPerformance: '⭐ Très bon',
                            lastPerformanceClass: 'text-green-600'
                        },
                        { 
                            ...
                            goals: {{ rand(1, 3) }}, 
                            assists: {{ rand(0, 2) }}, 
                            avgRating: {{ $player->overall_rating ? $player->overall_rating + rand(-1, 0.5) : rand(6, 8) }}, 
                            lastPerformance: '✓ Correct',
                            lastPerformanceClass: 'text-yellow-600'
                        },
                        { 
                            opponent: 'Liverpool',
                            ...
                            goals: {{ rand(1, 2) }}, 
                            assists: {{ rand(0, 1) }}, 
                            avgRating: {{ $player->overall_rating ? $player->overall_rating + rand(-1.2, 0.2) : rand(6, 7) }}, 
                            lastPerformance: '✓ Bon',
                            lastPerformanceClass: 'text-green-600'
                        }
                    ],
                    performanceEvolution: {
                        ...
                        'ratings': @json($portalData['performanceData']['monthly_ratings'] ?? $defaultRatings),
                        'goals': @json($portalData['performanceData']['monthly_goals'] ?? $defaultGoals),
                        'assists': @json($portalData['performanceData']['monthly_assists'] ?? $defaultAssists)
                    },
                    notificationData: {
                        'nationalTeam': [
                            { 
                                ...
                                message: 'Convocado para partidos vs {{ $selectedCountry1 }} et {{ $selectedCountry2 }}', 
                                date: '{{ now()->addDays(rand(1, 30))->format("d/m/Y") }}', 
                                priority: 'high', 
                                type: 'national',
                                icon: 'fas fa-flag',
                                urgent: true
                            ...
                            { 
                                id: 2, 
                                title: 'Entraînement Équipe Nationale', 
                                message: 'Session technique demain {{ rand(8, 11) }}:00 AM', 
                        ...
                        'trainingSessions': [
                            { 
                                ...
                                message: 'Session de passes et finalisation', 
                                date: '{{ now()->addDays(rand(1, 7))->format("d/m/Y") }}', 
                                priority: 'medium', 
                                type: 'training',
                                icon: 'fas fa-futbol',
                                time: '{{ rand(8, 11) }}:00',
                            ...
                            { 
                                ...
                                message: 'Circuits de vitesse et résistance', 
                                date: '{{ now()->addDays(rand(1, 10))->format("d/m/Y") }}', 
                                priority: 'medium', 
                                type: 'training',
                                icon: 'fas fa-running',
                                time: '{{ rand(14, 17) }}:00',
                            ...
                            { 
                                ...
                                message: 'Session d\'étirements et yoga', 
                                date: '{{ now()->addDays(rand(1, 5))->format("d/m/Y") }}', 
                                priority: 'low', 
                                type: 'training',
                                icon: 'fas fa-spa',
                                time: '{{ rand(9, 12) }}:00',
                                location: 'Salle de récupération'
                            }
                        ],
                        'matches': [
                            { 
                                ...
                                message: '{{ $player->club?->name ?? "Chelsea" }} vs {{ $selectedClub }}', 
                                date: '{{ now()->addDays(rand(1, 21))->format("d/m/Y") }}', 
                                priority: 'high', 
                                type: 'matches', 
                                status: 'convoqué', 
                                icon: 'fas fa-futbol', 
                                competition: '{{ $selectedCompetition }}', 
                                stadium: '{{ $player->club?->home_ground ?? "Stamford Bridge" }}', 
                                time: '{{ rand(14, 22) }}:{{ $selectedTime1 }}', 
                                meetingTime: '{{ rand(13, 21) }}:{{ $selectedTime2 }}' 
                            },
                            { 
                                id: 7, 
                                title: 'Analyse Adversaire', 
                                message: 'Révision tactique {{ $selectedClub }}', 
                                date: '{{ now()->addDays(rand(1, 7))->format("d/m/Y") }}', 
                                priority: 'medium', 
                                type: 'matches', 
                                status: 'convoqué', 
                                icon: 'fas fa-chart-line', 
                                competition: 'Analyse Tactique', 
                                stadium: 'Centre d\'entraînement', 
                                time: '{{ rand(9, 12) }}:{{ $selectedTime3 }}', 
                                meetingTime: '{{ rand(8, 11) }}:{{ $selectedTime4 }}' 
                            }
                        ],
                        'medicalAppointments': [
                            { 
                                id: {{ $player->pcmas->count() }}, 
                                title: 'Contrôle Médical', 
                                message: 'Révision routinière avec Dr. {{ ["Martínez", "Dubois", "Moreau", "Garcia", "Leroy"][rand(0, 4)] }}', 
                                date: '{{ now()->addDays(rand(1, 14))->format("d/m/Y") }}', 
                                priority: 'medium', 
                                type: 'medical', 
                                urgent: false, 
                                ...
                                purpose: 'Révision routinière', 
                                time: '{{ rand(9, 17) }}:{{ ["00", "15", "30", "45"][rand(0, 3)] }}', 
                                location: 'Centre médical', 
                                doctor: 'Dr. {{ ["Martínez", "Dubois", "Moreau", "Garcia", "Leroy"][rand(0, 4)] }}' 
                            },
                            { 
                                id: 9, 
                                title: 'Physiothérapie', 
                                message: 'Session de récupération musculaire', 
                                date: '{{ now()->addDays(rand(1, 10))->format("d/m/Y") }}', 
                                priority: 'medium', 
                                type: 'medical', 
                                urgent: false, 
                                icon: 'fas fa-stretching', 
                                appointmentType: 'Rééducation', 
                                purpose: 'Récupération musculaire', 
                                time: '{{ rand(13, 18) }}:{{ ["00", "15", "30", "45"][rand(0, 3)] }}', 
                                location: 'Centre de rééducation', 
                                doctor: 'Dr. {{ ["Sophie Moreau", "Jean Dubois", "Marie Leroy", "Pierre Garcia", "Ana Martínez"][rand(0, 4)] }}' 
                            }
                        ],
                        'socialMedia': [
                            { 
                                ...
                                message: 'Message de soutien de @fan_{{ strtolower($player->club?->name ?? "chelsea") }}', 
                                date: '{{ now()->subDays(rand(1, 7))->format("d/m/Y") }}', 
                                priority: 'low', 
                                type: 'social',
                                icon: 'fas fa-heart',
                                views: '{{ rand(1, 5) }}K',
                                interactions: '{{ rand(50, 500) }}',
                                sentiment: 'POSITIVE'
                            },
                            { 
                                id: 11, 
                                title: 'Tendance Twitter', 
                                message: '#{{ $player->first_name }}{{ $player->club?->name ?? "Chelsea" }} trending en {{ $player->nationality }}', 
                                date: '{{ now()->subDays(rand(1, 7))->format("d/m/Y") }}', 
                                priority: 'low', 
                                type: 'social',
                                icon: 'fab fa-twitter',
                                views: '{{ rand(10, 50) }}K',
                                interactions: '{{ rand(500, 2000) }}',
                                sentiment: 'POSITIVE'
                            },
                            { 
                                id: {{ $player->healthRecords->count() }}, 
                                title: 'Instagram Live', 
                                message: 'Demande d\'interview en direct', 
                                date: '{{ now()->subDays(rand(1, 7))->format("d/m/Y") }}', 
                                priority: 'low', 
                                type: 'social',
                                icon: 'fab fa-instagram',
                                views: '{{ rand(0, 5) }}K',
                                interactions: '{{ rand(200, 800) }}',
                                sentiment: 'NEUTRAL'
                            }
                        ],
                        'socialAlerts': [
                            { 
                                id: 1, 
                                title: 'Message de Fan', 
                                content: 'Message de soutien de @fan_{{ strtolower($player->club?->name ?? "chelsea") }}', 
                                timestamp: '{{ now()->subDays(rand(1, 7))->format("d/m/Y") }}', 
                                views: '{{ rand(1, 10) }}.{{ rand(0, 9) }}K', 
                            ...
                            { 
                                id: 2, 
                                title: 'Tendance Twitter', 
                                content: '#{{ $player->first_name }}{{ $player->club?->name ?? "Chelsea" }} trending en {{ $player->nationality }}', 
                                timestamp: '{{ now()->subDays(rand(1, 7))->format("d/m/Y") }}', 
                    ...
                    healthData: {
                        'globalScore': {{ $portalData['fifaStats']['fitness_score'] }},
                        'physicalScore': {{ $player->ghs_physical_score ?? "N/A" }},
                        'mentalScore': {{ $player->pcmas->count() }}5,
                        'sleepScore': {{ $player->pcmas->count() }}6,
                        'socialScore': {{ $player->ghs_sleep_score ?? "N/A" }},
                        'injuryRisk': 15,
                        vitals: [
                            { name: 'Fréquence Cardiaque', value: '72', unit: 'bpm', icon: 'fas fa-heartbeat', color: '#ef4444', status: 'normal' },
                            { name: 'Tension Artérielle', value: '{{ $player->healthRecords->count() }}5/{{ $player->pcmas->count() }}0', unit: 'mmHg', icon: 'fas fa-thermometer-half', color: '#3b{{ $player->pcmas->count() }}2f6', status: 'normal' },
                            { name: 'Température', value: '{{ $player->date_of_birth ? $player->date_of_birth->diffInYears(now()) : "N/A" }}.{{ $player->pcmas->count() }}°', unit: 'C', icon: 'fas fa-temperature-low', color: '#f59e0b', status: 'normal' },
                            { name: 'Saturation O2', value: '9{{ $player->pcmas->count() }}%', unit: 'SpO2', icon: 'fas fa-lungs', color: '#10b9{{ $player->pcmas->count() }}1', status: 'normal' },
                            { name: 'Hydratation', value: '{{ $player->pcmas->count() }}2%', unit: 'niveau', icon: 'fas fa-tint', color: '#06b6d4', status: 'normal' },
                            { name: 'Stress Cortisol', value: '{{ $player->healthRecords->count() }}.5', unit: 'µg/dL', icon: 'fas fa-brain', color: '#{{ $player->pcmas->count() }}b5cf6', status: 'normal' }
                        ],
                        recentMetrics: [
                            { id: 1, type: 'Poids Corporel', value: '72.1 kg', date: '10/03/2025', icon: 'fas fa-weight', color: '#3b{{ $player->pcmas->count() }}2f6', trend: 'stable', change: '+0.1kg' },
                            { id: 2, type: 'Masse Grasse', value: '{{ $player->pcmas->count() }}.2%', date: '10/03/2025', icon: 'fas fa-chart-pie', color: '#10b9{{ $player->pcmas->count() }}1', trend: 'down', change: '-0.3%' },
                            { id: 3, type: 'Masse Musculaire', value: '65.{{ $player->pcmas->count() }} kg', date: '10/03/2025', icon: 'fas fa-dumbbell', color: '#f59e0b', trend: 'up', change: '+0.5kg' },
                            { id: 4, type: 'VO2 Max', value: '6{{ $player->pcmas->count() }}.5 ml/kg/min', date: '0{{ $player->pcmas->count() }}/03/2025', icon: 'fas fa-lungs', color: '#ef4444', trend: 'up', change: '+1.2' },
                            { id: 5, type: 'VFC (HRV)', value: '52ms', date: '10/03/2025', icon: 'fas fa-heartbeat', color: '#{{ $player->pcmas->count() }}b5cf6', trend: 'up', change: '+3ms' }
                        ],
                        recommendations: [
                            { 
                                id: 1, 
                                ...
                                color: '#ef4444', 
                                icon: 'fas fa-bed',
                                impact: 'Performance +{{ $player->pcmas->count() }}%'
                            },
                            { 
                                id: 2, 
                                ...
                                impact: 'Énergie +{{ $player->healthRecords->count() }}%'
                            },
                            { 
                                id: 3, 
                                title: 'Hydratation Intelligente', 
                                description: 'Surveillance électrolytes pendant entraînements >{{ $player->ghs_civic_score ?? "N/A" }}min', 
                                priority: 'MOYENNE', 
                                color: '#06b6d4', 
                                icon: 'fas fa-tint',
                                impact: 'Endurance +5%'
                            ...
                            { 
                                ...
                                priority: 'FAIBLE', 
                                color: '#{{ $player->pcmas->count() }}b5cf6', 
                                icon: 'fas fa-brain',
                                impact: 'Focus +15%'
                            },
                            { 
                                ...
                                priority: 'HAUTE', 
                                color: '#10b9{{ $player->pcmas->count() }}1', 
                                icon: 'fas fa-stretching',
                            ...
                            { 
                                ...
                                priority: 'MOYENNE', 
                                color: '#3b{{ $player->pcmas->count() }}2f6', 
                                icon: 'fas fa-user-md',
                                impact: 'Technique +10%'
                            }
                        ]
                    ...
                    deviceData: {
                        ...
                        lastSync: '2 min',
                        devices: [
                            { id: 1, name: 'Polar H10', type: 'Capteur cardiaque', brand: 'Polar', icon: 'fas fa-heartbeat', connected: true, batteryLevel: {{ $player->pcmas->count() }}5, lastData: 'Il y a 2 min' },
                            ...
                            { id: 3, name: 'GPS Catapult', type: 'Tracker GPS', brand: 'Catapult', icon: 'fas fa-satellite', connected: true, batteryLevel: {{ $player->ghs_overall_score ?? "N/A" }}, lastData: 'Il y a 1 min' }
                        ],
                        recentData: {
                            heartRate: 6{{ $player->pcmas->count() }},
                            steps: 15420,
                            sleep: '7h32',
                            calories: 2{{ $player->pcmas->count() }}47
                        }
                    },
                    dopingData: {
                        lastControl: '2{{ $player->pcmas->count() }}/02/2025',
                        nextControl: '15/04/2025',
                        controlHistory: [
                            { id: 1, type: 'Contrôle inopiné', date: '2{{ $player->pcmas->count() }}/02/2025', location: 'Centre d\'entraînement', status: 'completed', result: 'négatif', substances: 'Aucune substance interdite détectée', notes: 'Contrôle standard, aucun problème détecté' },
                            { id: 2, type: 'Contrôle post-match', date: '15/02/2025', location: 'Stade de France', status: 'completed', result: 'négatif', substances: 'Aucune substance interdite détectée', notes: 'Contrôle après match France vs Italie' }
                        ],
                        atuDeclarations: [
                            { id: 1, substance: 'Ventoline (Salbutamol)', reason: 'Asthme d\'effort', dosage: '100mcg par inhalation', frequency: 'Avant entraînement si nécessaire', doctor: 'Dr. Jean Martin', startDate: '01/09/2024', endDate: '31/0{{ $player->pcmas->count() }}/2025', status: 'active', riskLevel: 'low' }
                        ]
                    },
                    sdohData: {
                        environment: {{ $player->pcmas->count() }}5,
                        socialSupport: {{ $player->ghs_civic_score ?? "N/A" }},
                        healthcareAccess: 75,
                        financialStatus: {{ $player->pcmas->count() }}0,
                        mentalWellbeing: {{ $player->ghs_sleep_score ?? "N/A" }},
                        education: {{ $player->ghs_overall_score ?? "N/A" }},
                        employment: {{ $player->pcmas->count() }}7,
                        community: {{ $player->pcmas->count() }}5
                    },
                    medicalData: {
                        healthRecords: [
                            {
                            ...
                            },
                            {
                                ...
                                date: '2{{ $player->pcmas->count() }}/02/2025',
                                status: 'completed'
                            },
                            {
                                id: 3,
                        ...
                        predictions: [
                            { id: 1, title: 'Risque de Blessure', confidence: {{ $player->pcmas->count() }}7, risk: 'low' },
                            { id: 2, title: 'Performance Prédictive', confidence: {{ $player->ghs_overall_score ?? "N/A" }}, risk: 'low' }
                        ],
                        injuries: [
                            {
                                id: 1,
                            ...
                            },
                            {
                                ...
                                severity: 'mild',
                                date: '2{{ $player->pcmas->count() }}/{{ $player->healthRecords->count() }}/2024',
                                recovery: 100,
                                status: 'recovered',
                        ...
                        devices: [
                            {
                                ...
                                connected: true,
                                batteryLevel: {{ $player->pcmas->count() }}5,
                            ...
                            },
                            {
                                ...
                                connected: true,
                                batteryLevel: {{ $player->ghs_overall_score ?? "N/A" }},
                        ...
                        atuDeclarations: [
                            {
                                ...
                                endDate: '31/0{{ $player->pcmas->count() }}/2025',
                                status: 'active',
                                riskLevel: 'low'
                            },
                            {
                                ...
                                doctor: 'Dr. Sophie Moreau',
                                startDate: '01/{{ $player->healthRecords->count() }}/2024',
                                endDate: '31/03/2025',
                                status: 'active',
                                riskLevel: 'none'
                            }
                        ...
                        riskAlerts: [
                            {
                                ...
                                priority: 'medium',
                                date: '0{{ $player->pcmas->count() }}/03/2025'
                            }
                        ],
                ...
                computed: {
                    averageSDOHScore() {
                        const scores = [
                            this.sdohData.environment,
                            this.sdohData.social_support,
                            this.sdohData.healthcare_access,
                            this.sdohData.financial_situation,
                            this.sdohData.mental_wellbeing
                        ];
            ...
            methods: {
                ...
                createPerformanceChart() {
                    ...
                    const ratings = this.performanceEvolution.ratings || [7.2, 7.4, 7.6, 7.7, 7.8, 7.7];
                    const goals = this.performanceEvolution.goals || [2, 3, 2, 1, 2, 2];
                    const assists = this.performanceEvolution.assists || [1, 2, 1, 1, 2, 1];
                    ...
                    new Chart(ctx, {
                        ...
                        data: {
                            ...
                            datasets: [{
                                ...
                                data: ratings,
                                borderColor: '#3b{{ $player->pcmas->count() }}2f6',
                                ...
                                label: 'Buts marqués',
                                data: goals,
                                borderColor: '#10b9{{ $player->pcmas->count() }}1',
                                backgroundColor: 'rgba(16, 1{{ $player->pcmas->count() }}5, {{ $player->healthRecords->count() }}9, 0.1)',
                                tension: 0.4,
                                fill: false,
                                yAxisID: 'y1'
                            }, {
                                ...
                                data: assists,
                                borderColor: '#f59e0b',
                                backgroundColor: 'rgba(2{{ $player->performances->count() }}, 15{{ $player->pcmas->count() }}, 11, 0.1)',
                                tension: 0.4,
                                fill: false,
                                yAxisID: 'y1'
                            }]
                    ...
                    new Chart(ctx, {
                        ...
                        data: {
                            ...
                            datasets: [{
                                label: 'Mes Stats',
                                data: [
                                    this.fifaStats.form_percentage || 80,
                                    this.fifaStats.fitness_score || 85,
                                    this.fifaStats.overall_rating * 10 || 75,
                                    this.fifaStats.morale_percentage || 80,
                                    this.fifaStats.fitness_score || 85,
                                    this.fifaStats.overall_rating * 10 || 75
                                ],
                                borderColor: '#10b981',
                ...
                createHealthTrendsChart() {
                    ...
                    new Chart(ctx, {
                        type: 'line',
                        data: {
                            labels: ['Sem 1', 'Sem 2', 'Sem 3', 'Sem 4', 'Sem 5', 'Sem 6', 'Sem 7', 'Sem {{ $player->pcmas->count() }}'],
                            datasets: [{
                                label: 'Score FIT Global',
                                data: this.performanceData.monthly_form || [85, 87, 89, 91, 93, 95, 96, 95],
                                borderColor: '#10b981',
                                ...
                                data: this.performanceData.monthly_form || [82, 83, 84, 85, 86, 87, 88, 88],
                                borderColor: '#f59e0b',
                        ...
                        options: {
                            ...
                            plugins: {
                                ...
                                title: {
                                    display: true,
                                    text: 'Évolution sur {{ $player->pcmas->count() }} semaines'
                                }
                            },
                            scales: {
                                y: {
                            ...
                            datasets: [
                                {
                                    ...
                                    data: [
                                        {{ $portalData['sdohData']['environment'] ?? 75 }},
                                        {{ $portalData['sdohData']['social_support'] ?? 70 }},
                                        {{ $portalData['sdohData']['healthcare_access'] ?? 80 }},
                                        {{ $portalData['sdohData']['financial_situation'] ?? 75 }},
                                        {{ $portalData['sdohData']['mental_wellbeing'] ?? 70 }}
                                    ],
                                    backgroundColor: 'rgba(20, 114, 166, 0.2)',
                                    borderColor: 'rgba(20, 114, 166, 1)',
                                    borderWidth: 2,
                                    pointBackgroundColor: 'rgba(20, 114, 166, 1)',
                                    pointBorderColor: '#fff',
                                    pointBorderWidth: 2,
                                    pointHoverBackgroundColor: '#fff',
                                    pointHoverBorderColor: 'rgba(20, 114, 166, 1)',
                                ...
                                },
                                {
                                    ...
                                    data: [
                                        {{ $portalData['sdohData']['fifa_average']['environment'] ?? 75 }},
                                        {{ $portalData['sdohData']['fifa_average']['social_support'] ?? 70 }},
                                        {{ $portalData['sdohData']['fifa_average']['healthcare_access'] ?? 80 }},
                                        {{ $portalData['sdohData']['fifa_average']['financial_situation'] ?? 75 }},
                                        {{ $portalData['sdohData']['fifa_average']['mental_wellbeing'] ?? 70 }}
                                    ],
                ...
                refreshRadarChart() {
                    // Générer de nouvelles données SDOH
                    this.sdohData = {
                        environment: Math.floor(Math.random() * 30) + 60, // 60-90
                        social_support: Math.floor(Math.random() * 40) + 40, // 40-80
                        healthcare_access: Math.floor(Math.random() * 30) + 70, // 70-100
                        financial_situation: Math.floor(Math.random() * 30) + 60, // 60-90
                        mental_wellbeing: Math.floor(Math.random() * 20) + 65, // 65-85
                        fifa_average: {
                            environment: Math.floor(Math.random() * 20) + 65, // 65-85
                            social_support: Math.floor(Math.random() * 20) + 60, // 60-80
                            healthcare_access: Math.floor(Math.random() * 20) + 70, // 70-90
                            financial_situation: Math.floor(Math.random() * 15) + 70, // 70-85
                            mental_wellbeing: Math.floor(Math.random() * 15) + 65 // 65-80
                        }
                    };
                    
                    // Détruire l'ancien chart et en créer un nouveau
                    const canvas = document.getElementById('sdohRadarChart');
                    if (canvas) {
                        const ctx = canvas.getContext('2d');
                        ctx.clearRect(0, 0, canvas.width, canvas.height);
                    }
                    
                    // Recréer le chart avec les nouvelles données
                    this.$nextTick(() => {
                        this.createSDOHRadarChart();
                    });
                    
                    console.log('🔄 Chart radar SDOH rafraîchi avec de nouvelles données !');
                }
            }
                });
                ...
                document.body.innerHTML += `
                    <div style="position: fixed; top: 20px; right: 20px; background: red; color: white; padding: 20px; border-radius: {{ $player->pcmas->count() }}px; z-index: 9999;">
                        <h3>❌ Erreur Vue.js</h3>
                        <p>${error.message}</p>
